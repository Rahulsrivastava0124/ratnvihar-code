...
import { Container, Row, Col, Modal, Form, Button, Dropdown, InputGroup } from 'react-bootstrap';
...
import { isEmpty, priceFormat, displayAmount, setLastVisitPage, convertUnitToGram, weightFormat } from 'src/helpers/helper';
...
class ProductsPage extends React.Component {
    ...
    componentDidUpdate(prevProps) {
        ...
        if (this.props.query.get('category') != prevProps.query.get('category') || this.props.query.get('subcategory') != prevProps.query.get('subcategory') || this.props.query.get('search') != prevProps.query.get('search') || this.props.query.get('offer') != prevProps.query.get('offer') || this.props.query.get('recent_view') != prevProps.query.get('recent_view')) {
    ...
    loadProducts = (search) => {
        ...
        productList({...params, page: 1, limit: limit})
        .then(res => {
            if(res.data.success){
                this.setState({
                    ...
                    this.softLoadProducts({...params, page: 2, limit: limit}, res.data.data.total);
                });
            }
        })
    }
                ...
                if(res.data.success){
                    this.setState({
                        ...
                        if(totalPage >= (params.page + 1)){
                            this.softLoadProducts({...params, page: (params.page + 1)}, res.data.data.total);
                        }
                    });
                }
            })
    ...
    wishlistHandler = async (product) => {
        ...
        if (response.data.success) {
            ...
            if(res.data.success){
                this.props.dispatch({type: CUSTOMER_PRODUCT_WISHLIST_UPDATE, payload: res.data.data});
    ...
    getProducts = () => {
        ...
        if (this.state.sort_by == "price_low_high") {
            products.sort((a, b) => parseFloat(a.sale_price) - parseFloat(b.sale_price));
        } else if (this.state.sort_by == "price_high_low") {
            products.sort((a, b) => parseFloat(b.sale_price) - parseFloat(a.sale_price));
        } else if (this.state.sort_by == "discount") {
            products.sort((a, b) => parseFloat(b.total_save) - parseFloat(a.total_save));
        }
        return products;
    }

    getSearchBy = () => {
        if (this.props.query.get('category') && this.props.query.get('subcategory')) {
    ...
    render() {
        ...
        return (
            <div className='search-wrapper'>
                <Container>
                    ...
                    <div className='breadcrumb-wrapper'>
                        <Breadcrumb>
                            ...
                            <Breadcrumb.Item active>{searchBy}</Breadcrumb.Item>
                        </Breadcrumb>
                    </div>
                    {
                        banner ? 
                    ...
                    <div className='products_header_title'>
                        <ul>
                            <li><h1>{searchBy}</h1></li>
                            <li>|</li>
                            <li><span> {this.state.total} </span> DESIGNS</li>
                        </ul>
                    ...
                    <div className='search-area desktop-search'>
                        <InputGroup className="">
                            ...
                            />
                            <InputGroup.Text id="basic-addon2" style={{ cursor: 'pointer' }} onClick={this.handleSearch}><BiSearchAlt2 /></InputGroup.Text>
                        </InputGroup>
                        <div className='filter-button'>
                            ...
                            <Dropdown>
                                ...
                                <Dropdown.Menu>
                                    <Dropdown.Item name='whats_new' onClick={(e) => this.handlesortBy(e)}>{this.state.sort_by == 'whats_new' ? <BsCheck2 /> : ''} What's New</Dropdown.Item>
                                    <Dropdown.Item name='popular' onClick={(e) => this.handlesortBy(e)}>{this.state.sort_by == 'popular' ? <BsCheck2 /> : ''} Popular</Dropdown.Item>
                                    <Dropdown.Item name='price_low_high' onClick={(e) => this.handlesortBy(e)}>{this.state.sort_by == 'price_low_high' ? <BsCheck2 /> : ''} Price Low to High</Dropdown.Item>
                                    <Dropdown.Item name='price_high_low' onClick={(e) => this.handlesortBy(e)}>{this.state.sort_by == 'price_high_low' ? <BsCheck2 /> : ''} Price High to Low</Dropdown.Item>
                                    <Dropdown.Item name='discount' onClick={(e) => this.handlesortBy(e)}>{this.state.sort_by == 'discount' ? <BsCheck2 /> : ''} Discount</Dropdown.Item>
                                </Dropdown.Menu>
                            </Dropdown>
                        </div>
                    </div>
                    ...
                    <div className='mobile-search'>
                        <InputGroup className="">
                            <Form.Control
                                ...
                                onChange={this.handleSearchChange}
                            />
                            <InputGroup.Text id="basic-addon2" onClick={this.handleSearch}><FiSearch /></InputGroup.Text>
                        </InputGroup>
                                                ...
                                                productList.map((product, index) => (
                                                    <div className='search-inner' key={index}>
                                                        <div className='s-slider-image'>
                                                            <img className="swap-on-hover__front-image" src={product.default_image} alt='feature product' onClick={() => this.handleProductDetails(product)} />
                                                            <div className='swap-on-hover__back-image'>
                                                                <img src={product.default_image} />
                                                                <div className='wishlist'>
                                                                    {
                                                                        product.has_wishlist ?
                                                                        <BsHeartFill onClick={() => this.wishlistHandler(product)} className="wishlist_active" />
                                                                        :
                                                                        <BsHeart onClick={() => this.wishlistHandler(product)} />
                                                                    }
                                                                </div>
                                                                {
                                                                    product.video != "" ?
                                                                        <div className='video_button'>
                                                                            <BsFillPlayFill onClick={() => this.openVideo(product)} />
                                                                        </div>
                                                                        : null
                                                                }
                                                            </div>
                                                            {
                                                                (product.making_charge_dis_percent > 0 || product.discount_percent > 0) ?
                                                            ...
                                                            <div className='search-button'>
                                                                <Button variant="primary" onClick={() => this.handleProductDetails(product)}>ADD TO CART</Button>{' '}
                                                                <Button variant="secondary" onClick={() => this.handleProductDetails(product)}>VIEW DETAILS</Button>{' '}
                                                            </div>
                                                        </div>
                                                            ...
                                                            <div className='ring-price'>
                                                                {
                                                                    product.have_offer ?
                                                                        <>
                                                                            ...
                                                                            <div>
                                                                                <span className='item-saving-price'> Save
                ...
                <Modal show={this.state.videoDialog} onHide={this.handleVideoClose} centered className="popup-product">
                    <Modal.Body>
                        {
                        ...
                        <div className='modal-video-buttons'>
                            <Button variant="secondary" onClick={() => this.handleProductDetails(this.state.video_product)}>VIEW DETAILS</Button>

                        </div>
                        <span className="close-popup" onClick={this.handleVideoClose}>X</span>
                    </Modal.Body>
                </Modal>
