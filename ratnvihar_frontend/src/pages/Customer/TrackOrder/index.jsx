                    ...
                    <>
                        {
                            !order ?
                                    ...
                                    <div className='track-order desktop-track-order'>
                                        <Container>
                                            ...
                                            <div className='track-header-wrapper'>
                                                <h1>Track Order {order.status != 'delivered' ? <span>Estimated Delivery: {order.expected_delivery_date}</span> : null}</h1>
                                                ...
                                                <div className='steps'>
                                                    {
                                                        order.status_progress.map((item, index) => {
                                                            return <div className={'step' + (item.is_active ? ' active' : '')} key={index}>
                                                                <span className='icon'>{getStatusIcon(item.status)}</span>
                                                                <div><h1>{item.status_display}{item.status == "pending" ? <span className='timer'><FcAlarmClock />Usually takes 30-40 mins</span> : null}</h1> {item.is_active ? <h3>{item.date}</h3> : null}</div>
                                                            </div>
                                                        })
                                                    }
                                                </div>
                                            ...
                                            <div className='track-order-footer mt-3 d-flex justify-content-between'>
                                                <div className='track-f-content'>
                                                    ...
                                                    <Button variant="primary" onClick={handleHelpcenter}>VISIT HELP CENTER</Button>
                                                </div>
                                ...
                                <HiArrowNarrowLeft /> <h3>Order Summary</h3>
                                            ...
                                            </div>
                                            {
                                                order.status != 'delivered' ?
                                                    <div className='track-header-wrapper mb-3'>
                                                        <h1> <span>Estimated Delivery: {order.expected_delivery_date}</span></h1>
                                                    </div>
                                                    : null
                                            }
                                    ...
                                    <Modal show={helpDialog} onHide={handleHelpDialogClose} centered className="popup-product">
                                        <Modal.Body>
                                            <div className='help_dialog'>
                                            {
                                                order ?
                                                <><div>{order.help_mobile} | {order.help_email}</div></>
                                                : null
                                            }
                                            </div>
                                            <span className="close-popup" onClick={handleHelpDialogClose}>X</span>
                                        </Modal.Body>
                                    </Modal>
                                </>
                        }
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(TrackOrder));