...
import { GoogleOAuthProvider, GoogleLogin as NewGoogleLogin, useGoogleLogin  } from '@react-oauth/google';
...
class Login extends React.Component {
    ...
    componentDidUpdate(prevProps) {
        if (this.state.isLoggedIn) {
            ...
            setLastVisitPage('');
            let url = lastVisitPage ? lastVisitPage.replace(process.env.BASE_URL, "/") : "/";
            let startFirstTwo = url.substring(0,2);
            url = (startFirstTwo == '//' || startFirstTwo == '///') ? url.substr(1) : url;
            console.log('url', url)
    ...
    render() {
        ...
        return (
            <div className='login-wrapper'>
                <Container>
                    <Row>
                        <Col xs={12} md={7} className='d-none d-sm-block'>
                            <div className='login-image'>
                               <span className='login-pro-inner'> <img src={LoginImage} className="login-profile" alt='' /></span>
                                <div className='login-header'>
                                <h5>Mision</h5>
                                <p>Ratnavihar is especially known as a wholesaler.Ratnavihar has a distinct identity in the world of Diamond Jewellery, Gemstones and Rudraksha, which is providing its service in many cities in India. </p>
                                
                                <p>The company has achieved a good footing of growth in a few years. We have endeavoured to provide seamless convenience to our customers, prices displayed are Cash on Delivery and our 100% refund policy is highly appreciated. We continuously strive to expand and make our service more convenient than ever before.</p>
                                
                                <p>My goal is to connect with every small and big city to reach the right things at the right price.</p>
                               <p> Your suggestion can be helpful for us, we look forward to your suggestion.
                                We are committed to quality and loyalty to our customers.
                                </p>
                                <h5>ABOUT US</h5>
                        ...
                        <Col xs={12} md={5}>
                            <div className='login-form-wrapper'>
                                ...
                                <form onSubmit={this.onSubmit}>
                                    <Form.Group className="mb-4 mt-3" controlId="formEmailAddress">
                                    ...
                                    <Form.Group className="mb-2 position-relative" controlId="formBasicPassword">
                                        <Form.Control name="password" onChange={(e) => this.handleChange(e)} value={formValaues.password} type={passwordShow ? "text" : "password"} placeholder="Enter Password"  required />
                                       
                                        <span type='invalid' style={{ color: "red" }}> {formErrors.password}  </span>
                                        <span className='show-pass'>{!this.state.passwordShow ? <AiOutlineEyeInvisible onClick={() => this.setState({passwordShow: true})} /> : <AiOutlineEye onClick={() => this.setState({passwordShow: false})} /> }</span>
                                    ...
                                    <p className='login-text mt-2 mb-2'>Do not have an account with us? <a href="/signup">Sign Up</a></p>
                                    <hr />
                                    ...
                                    <div className='login-footer-button'>
                                        ...
                                        <GoogleLogin
                                            clientId={"701708035160-86p8d20lg8etmlk4r8ltck35mnua681r.apps.googleusercontent.com"}
                                            render={renderProps => (
                                                <button onClick={renderProps.onClick} className="btn btn-google bg-danger email-btn">{this.state.googleBtnTxt}</button>
                                            )}
                                            onSuccess={this.handleGoogleRespone}
                                            onFailure={this.handleGoogleRespone}
                                            cookiePolicy={'single_host_origin'}
                                        />
                                        <Button variant="primary" className='fb-btn' href=""><FaFacebookF /> &nbsp; LOGIN WITH FACEBOOK</Button>
                                    </div>
                                </form>
