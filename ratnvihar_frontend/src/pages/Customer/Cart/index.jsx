...
import { isEmpty, priceFormat, displayAmount, convertUnitToGram, weightFormat, isSalesExecutive, isRetailer, setLastVisitPage } from 'src/helpers/helper';
...
class CartPage extends React.Component {
    ...
    handlPurityChange = (cartId, materialId, purityId) => {
        ...
        this.props.actions.CartUpdateSizeMaterial(cartId, { material_id: materialId, purity_id: purityId });
    }
    ...
    render() {
        ...
        return (
            <>
                ...
                >
                    <div>
                        <Modal className='delete-popup' show={this.state.removeDialog} onHide={this.handleRemoveDialogClose}>
                            <Modal.Header closeButton>
                                <Modal.Title>Remove From Cart</Modal.Title>
                            </Modal.Header>
                            <Modal.Body>Are you want to remove this product from cart ?</Modal.Body>
                            <Modal.Footer>
                                <Button variant="secondary" className='close-btn' onClick={this.handleRemoveDialogClose}>
                                    No
                                </Button>
                                <Button variant="danger" className='delete-btn' onClick={this.handleCartRemove}>
                                    Yes
                                </Button>
                            </Modal.Footer>
                        </Modal>
                                        ...
                                        {cartList.length > 0 ?
                                            cartList.map((val, index) => (
                                                <React.Fragment key={index}>
                                                        ...
                                                        <div className='cart-inner-wrapper' key={index}>
                                                            <div className='cart-inner'>
                                                                <div className='cart-image'>
                                                                    <Link to={"/products/" + val.product_slug}><img src={val.product_image} alt='' /></Link>
                                                                </div>
                                                                <div className='cart-image-content'>
                                                                    <div>
                                                                        <div className='cart-price-title-wrapper'>
                                                                            <div className='cart-image-title'>
                                                                                <h2><Link to={"/products/" + val.product_slug}>{val.product_name}</Link></h2>
                                                                            ...
                                                                            <div className='price-wrapper'>
                                                                                <div className='cart-original-price'>
                                                                                    {
                                                                                        val.have_offer ?
                                                                                            <span className='strikethrough'>{val.total_price_without_dis_display}</span>
                                                                                            : null
                                                                                    }
                                                                                </div>
                                                                        ...
                                                                        <div className='purity-wrapper'>
                                                                            <Form.Group>
                                                                                {
                                                                                    this.getSizeMaterial(val).materials.map((item, key) => (
                                                                                        <Row key={key}>
                                                                                            <Col md={6}>
                                                                                                <Form.Label>{this.getSizeMaterial(val).materials.length > 1 ? item.material_name : "Purity"}:</Form.Label>
                                                                                            </Col>
                                                                                            <Col md={6}>
                                                                                                <Form.Select value={this.getSelectedPurity(val, item.material_id)} onChange={(e) => this.handlPurityChange(val.id, item.material_id, e.target.value)} className={this.getSelectedPurity(val, item.material_id) == '' ? 'error_input' : ''}>
                                                                                                    {
                                                                                                        this.getSelectedPurity(val, item.material_id) == '' ?
                                                                                                            <option value=""></option>
                                                                                                            : null
                                                                                                    }
                                                                                                    {
                                                                                                        item.purities.map((i, k) => (
                                                                                                            <option value={i.id} key={k}>{i.name}{/*{!val.is_manual ? <>, {item.weight} {item.unit_name}</> : null}*/}</option>
                                                                                                        ))
                                                                                                    }
                                                                                                </Form.Select>
                                                                                            </Col>
                                                    ...
                                                    <Button variant="primary" onClick={() => this.removeConfirm(val)}> Remove</Button>
                                                    <Button variant="primary" onClick={() => this.handleMoveToWishlist(val)}> Move To Wishlist</Button>
                                                </div>
                                                                            {/*<div className='cart-icons'>
                                                                            ...
                                                                            <div className='cart-select'>
                                                                                {
                                                                                    val.product_type != "material" ?
                                                                                        <Form.Group>
                                                                                            ...
                                                                                            <Form.Select value={val.size_id} onChange={(e) => this.handleSizeChange(val, e.target.value)}>
                                                                                                {
                                                                                                    val.size_materials.map((i, k) => (
                                                                                                        <option value={i.size_id} key={k}>{i.size_name}</option>
                                                                                                    ))
                                                                                                }
                                                                                            </Form.Select>
                                                                                        </Form.Group>
                                                                                ...
                                                                                <Form.Group>
                                                                                    ...
                                                                                    <Form.Select value={val.quantity} onChange={(e) => this.handleQtyChange(val.id, e.target.value)}>
                                                                                        {
                                                                                            [...Array(10).keys()].map((i, k) => (
                                                                                                <option value={i + 1} key={k}>{i + 1}</option>
                                                                                            ))
                                                                                        }
                                                                                    </Form.Select>
                                                                                    {/*{val.quantity}*/}
                                                                                ...
                                                                                <Row>
                                                                                    <Col xs={6}>
                                                                                        <Form.Group className="mb-3">
                                                                                            ...
                                                                                            <InputGroup className={(this.getManualErr(index, 'weight') ? 'is-invalid error_input' : '')}>
                                                                                                <Form.Control type="text" placeholder="Enter weight" value={val.manual_weight} onChange={(e) => this.handleWeight(e, index)} />
                                                                                                <InputGroup.Text>{val.cart_material[0].unit_name}</InputGroup.Text>
                                                                                            </InputGroup>
                                                                                    ...
                                                                                    <Col xs={6}>
                                                                                        <Form.Group className="mb-3">
                                                                                            ...
                                                                                            <Form.Control type="text" placeholder="Enter quantity" value={val.manual_qty} onChange={(e) => this.handleQty(e, index)} className={(this.getManualErr(index, 'qty') ? 'is-invalid error_input' : '')} />
                                                                                            <Form.Control.Feedback type="invalid">
                                                                                                {this.getManualErr(index, 'qty')}
                                                                                            </Form.Control.Feedback>
                                                                                            <Form.Label>&nbsp;</Form.Label>
                                                                                            <Button variant="primary" className='dark_button' onClick={() => this.handleManulUpdate(index)}>Update</Button>
                                                                                            </span>
                                                                                            </Form.Group>
                                                                                    </Col>
                                                                                    <Col xs={6}>
                                        ...
                                        <div className='cart-right-sidebar'>
                                            ...
                                            </div>
                                            {
                                                !isEmpty(this.state.promocode) ?
                                                    <div className='summary-items'>
                                                        <p>Voucher code ( <span>{this.state.promocode} </span> )</p>
                                                ...
                                                cartList.length ?
                                                    <>
                                                                ...
                                                                !this.isRetailer && !this.isSalesExecutive ?
                                                                    <>
                                                                        <div className='voucher-wrapper'>
                                                                            <p className='' onClick={() => this.setState({ showPromocode: !this.state.showPromocode })} role="button">I have a voucher code/ gift card</p>
                                                                            <p className='show-offer' onClick={() => this.setState({ offerDialog: true })}> View Offers </p>
                                                                        </div>
                                                                    </>
                                                                    : null
                                                            }
                                                        </div>
                                                        {
                                                            this.state.showPromocode ?
                                                                <div className='promo-code'>
                                                                    <InputGroup className={"mb-0" + (this.state.code_err ? ' error_input' : '')}>
                                                                        <Form.Control
                                                                            placeholder="Enter Promo Code"
                                                                            value={this.state.code}
                                                                            onChange={(e) => this.setState({ code: e.target.value })}
                                                                        />
                                                                        <InputGroup.Text onClick={this.applyPromocode} role="button">APPLY</InputGroup.Text>
                                                                    </InputGroup>
                                                        ...
                                                        {/*<div className='p-policy mt-3 p-1 mb-3'>
                                                            ...
                                                            <div className='p-authenticity p-2 mt-3'>
                                        ...
                                        cartList.map((val, index) => (
                                            <Col xs={12} md={9} key={index}>
                                                <div className='cart-inner-wrapper'>
                                                    ...
                                                    {
                                                        !val.is_manual ?
                                                        <div className='cart-select mt-2 mb-2'>
                                                            {
                                                                val.product_type != "material" ?
                                                                    <Form.Group>
                                                                        <Form.Label>Size:</Form.Label>
                                                                    ...
                                                                    <Row key={key}>
                                                                        ...
                                                                        <Col xs={6} md={6}>
                                                                            <Form.Select value={this.getSelectedPurity(val, item.material_id)} onChange={(e) => this.handlPurityChange(val.id, item.material_id, e.target.value)} className={this.getSelectedPurity(val, item.material_id) == '' ? 'error_input' : ''}>
                                                                                ...
                                                                                {
                                                                                    item.purities.map((i, k) => (
                                                                                        <option value={i.id} key={k}>{i.name}{/*, {item.weight} {item.unit_name}*/}</option>
                                                                                    ))
                                                                                }
                                                                            </Form.Select>
                                                                        </Col>
                                                ...
                                                <>
                                                    ...
                                                    </div>
                                                    {
                                                        this.state.showPromocode ?
                                                            ...
                                                            <div className='p-authenticity p-2'>
                                                                <h4 className='text-center'>CERTIFICATE OF AUTHENTICITY</h4>
                                                                <ul className="authenticity_list">
                                                                <li><img src={au1} alt='' /></li>
                                                                <li><img src={au2} alt='' /></li>
                                                    ...
                                                    <span>Any Questions?</span> Please call us at +91 98744 45878
                                                        or Chat with us</p>
                    ...
                    <Modal show={this.state.offerDialog} onHide={this.handleOfferClose} centered className="popup-offers">
                        <h1 className='coupon-heading'>
                            Best Promo Codes
                        </h1>
                        <Modal.Body>
                                        ...
                                        <div className='coupon-wrapper' key={key}>
                                            ...
                                            <div className='coupon-offer-content'>
                                                <div className='coupon-title'>
                                                    <h2>{item.code}</h2>
                                                    <button variant='primary' onClick={() => this.handleCodeApply(item)}>APPLY</button>
                                                </div>
                            ...
                            <span className="close-popup" onClick={this.handleOfferClose}>X</span>
                        </Modal.Body>
                    </Modal>
                </LoadingOverlay>
            </>
...
const mapDispatchToProps = dispatch => ({
    dispatch,
    actions: bindActionCreators({ CartList, CartUpdate, CartDelete, checkoutList, CartUpdateSizeMaterial, CartApplyPromocode }, dispatch)
});
