...
import { productFetch, recentlyViewCategories, prodductReviews } from "actions/Customer/product.actions";
import { AddToCart, CartList, AddToCartRaw } from "actions/Customer/addcart.actions";
...
class ProductDetails extends React.Component {
    ...
    loadProductReviews = async (product_id) => {
        let res = await prodductReviews({ page: this.state.page, size: this.state.limit, product_id: product_id });
        if (res.data.success) {
            let review_data = this.state.page == 1 ? res.data.data : { ...this.state.review_data, ...res.data.data };
            this.setState({
    ...
    loadProduct = async () => {
        let response = await productFetch({ slug: this.props.params.slug, recently_view: 1 });
    ...
    handlePurityChange = (material_id, purity_id) => {
        ...
        let materialIndex = _.findIndex(sizeMaterial.materials, function (item) { return item.material_id == material_id });
        let purityIndex = _.findIndex(sizeMaterial.materials[materialIndex].purities, function (item) { return item.id == purity_id });
        for (let i = 0; i < sizeMaterial.materials[materialIndex].purities.length; i++) {
            ...
            if (i == purityIndex) {
                ...
                sizeMaterial.materials[materialIndex].discount_percent = sizeMaterial.materials[materialIndex].purities[i].discount_percent;
            }
        }

        sizeMaterial.materials[materialIndex].price = sizeMaterial.materials[materialIndex].purities[purityIndex].price;
        sizeMaterial.materials[materialIndex].mrp_price = sizeMaterial.materials[materialIndex].purities[purityIndex].mrp_price;
        let total_mrp_price = 0, total_sale_price = 0, total_gst = 0;
        for (let i = 0; i < sizeMaterial.materials.length; i++) {
            let m = _.filter(sizeMaterial.materials[i].purities, { is_selected: true });
        ...
        if (product.tax_info) {
            let igst = 0;
            let cgst = (!isEmpty(product.tax_info.cgst)) ? priceFormat(total_sale_price * parseFloat(product.tax_info.cgst) / 100, true) : 0;
            let sgst = (!isEmpty(product.tax_info.sgst)) ? priceFormat(total_sale_price * parseFloat(product.tax_info.sgst) / 100, true) : 0;
            let cgst_m = (!isEmpty(product.tax_info.cgst)) ? priceFormat(total_mrp_price * parseFloat(product.tax_info.cgst) / 100, true) : 0;
            let sgst_m = (!isEmpty(product.tax_info.sgst)) ? priceFormat(total_mrp_price * parseFloat(product.tax_info.sgst) / 100, true) : 0;
    ...
    handleAddToCart = async () => {
        ...
        const { product } = this.state;
        let weight = null, quantity = 1, is_manual = 0;
        ...
        let selected_materials = product ? product.size_materials[this.state.sizeMaterialIndex].materials : [];
        let size_id = product ? product.size_materials[this.state.sizeMaterialIndex].size_id : null;
        let rate = product ? product.size_materials[this.state.sizeMaterialIndex].sale_price : null;
        let total_weight = 0;
        let materials = [];
        ...
        for (let i = 0; i < selected_materials.length; i++) {
            ...
            let total_gram = convertUnitToGram(thisM.unit_name, weight ? weight : thisM.weight);
    ...
    render() {
        ...
        const sizeMaterial = product ? product.size_materials[this.state.sizeMaterialIndex] : null;
        const totalPage = review_data ? Math.ceil(review_data.total / this.state.limit) : 1;
                                            ...
                                            <Container>
                                                <div className='breadcrumb-wrapper'>
                                                    <Breadcrumb>
                                                        ...
                                                        <Breadcrumb.Item href={'/products?category=' + product.category_slug + '&subcategory=' + product.sub_category_slug}>{product.sub_category}</Breadcrumb.Item>
                                                                ...
                                                                <SwiperSlide className='p_details_slider_wrapper' style={{ maxHeight: '100px' }}>
                                                                    {
                                                                        (this.state.play_video && product.video != "") ?
                                                                            ...
                                                                            :
                                                                            <img className='p_details_slider' src={product.images[this.state.imageIndex]} />
                                                                    }
                                                                </SwiperSlide>
                                                            </Swiper>
                                                            <Swiper
                                                                ...
                                                                }
                                                                {
                                                                    product.images.map((item, key) => (
                                                                        <SwiperSlide className='slider-thumbnail' key={key}>
                                                                            <img className='thumnail-slider' src={item} onClick={() => this.changeSliderImage(key)} />
                                                                        </SwiperSlide>
                                                                    ))
                                                                }
                                                                ...
                                                                <Accordion.Item eventKey="0">
                                                                    ...
                                                                    <Accordion.Body>
                                                                        <div className='product-details-items-content'>
                                                                            ...
                                                                            <div className='product-details-items-item'>
                                                                                <span>Product Weight</span>  <span>{sizeMaterial.product_weight_display}</span>
                                                                            </div>
                                                                            {
                                                                                sizeMaterial.materials.map((item, key) => (
                                                                                    <React.Fragment key={key}>
                                                                                        <div className='product-details-items-item details-header'>
                                                                                            <span>{item.material_name.toUpperCase()} DETAILS</span>
                                                                                        </div>
                                                                                        <div className='product-details-items-item'>
                                                                                            <span>Total Weight</span>  <span>{item.weight} {item.unit_name}</span>
                                                                                        </div>
                                                                                        {
                                                                                            (!isEmpty(item.quantity) && parseInt(item.quantity) > 0) ?
                                                                                                <div className='product-details-items-item'>
                                                                                                    <span>Total No of {item.material_name.toUpperCase()}</span>  <span>{item.quantity}</span>
                                                                                                </div>
                                                                                                : null
                                                                                        }
                                                        ...
                                                        <div className='product-details-container'>
                                                            <div className='righ-side-wrapper'>
                                                                            ...
                                                                            <ul>
                                                                                {
                                                                                    [...Array(5).keys()].map((x, i) => (
                                                                                        <li className={product.rating < (x + 1) ? 'blank-star' : ''} key={i}><BsStarFill /></li>
                                                                                    ))
                                                                                }
                                                                                <li className='rating'> {product.rating.toFixed(2)}</li>
                                                                            </ul>
                                                                ...
                                                                <div className='p-header-price'>
                                                                    {
                                                                        sizeMaterial.discount_percent > 0 ?
                                                                            <h3> <span>{displayAmount(sizeMaterial.mrp_price)} </span>{sizeMaterial.discount_percent > 0 ? <sup className='discount'> {sizeMaterial.discount_percent}% OFF </sup> : null} </h3>
                                                                            :
                                                                            null
                                                                    }
                                                                    <h2>
                                                                        {displayAmount(sizeMaterial.sale_price)}
                                                                        {!product.certified && product.type == "material" ? (" / " + sizeMaterial.materials[0].unit_name) : ''}
                                                                    ...
                                                                    <div className='breakup-content'>
                                                                        {
                                                                            sizeMaterial.materials.map((item, key) => (
                                                                                <div className='breakup-item' key={key}>
                                                                                    <span>{item.material_name} &nbsp;&nbsp;&nbsp; {item.discount_percent > 0 ? (item.discount_percent + '% OFF') : ''}</span>  <span>{item.mrp_price > item.price ? <span className='line-through'>
                                                                                        {displayAmount(item.mrp_price)}
                                                                                    </span> : null}{displayAmount(item.price)}</span>
                                                                                </div>
                                                                            ))
                                                                        }

                                                                        <div className='breakup-item'>
                                                                            <span>Making Charge {sizeMaterial.making_charge_dis_percent > 0 ? (sizeMaterial.making_charge_dis_percent + '% OFF') : ''}</span>  <span>{sizeMaterial.making_charge_mrp > sizeMaterial.making_charge ? <span className='line-through'>
                                                                                {displayAmount(sizeMaterial.making_charge_mrp)}
                                                                            </span> : null}{displayAmount(sizeMaterial.making_charge)}</span>
                                                                        </div>
                                                                        <div className='breakup-item'>
                                                                            <span>GST</span>  <span>{displayAmount(sizeMaterial.total_gst)}</span>
                                                                        </div>
                                                                        <hr style={{ margin: '8px 0' }} />
                                                                        <div className='breakup-item'>
                                                                            <span className='total'>Total</span>  <span className='total'>{displayAmount(sizeMaterial.sale_price * cartWeight)} </span>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                                <div className='p-customize-design mb-4'>
                                                                    <Accordion defaultActiveKey="0" alwaysOpen>
                                                                        <Accordion.Item eventKey="0">
                                                                            <Accordion.Header>Customize Your Product</Accordion.Header>
                                                                            <Accordion.Body>
                                                                                {
                                                                                    product.type != "material" ?
                                                                                        <div className='p-size'>
                                                                                            <span>Select Size :</span>
                                                                                            <DropdownButton title={sizeMaterial.size_name} name='size'
                                                                                                key={'Secondary'}
                                                                                                id={`dropdown-variants-Secondary`}
                                                                                                variant={'Secondary'}>
                                                                                                {
                                                                                                    product.size_materials.map((item, index) =>
                                                                                                    (<div key={index}>
                                                                                                        <Dropdown.Item onClick={() => this.handleSizeChange(index)}>{item.size_name}</Dropdown.Item>
                                                                                                    </div>)
                                                                                                    )}
                                                                                            </DropdownButton>
                                                                                        </div>
                                                                                ...
                                                                                }
                                                                                {
                                                                                    sizeMaterial.materials.map((item, key) => (
                                                                                        <div className='p-purity' key={key}>
                                                                                            <span>{sizeMaterial.materials.length > 1 ? item.material_name : "Purity"}:
                                                                                                <Form >
                                                                                                    <div key={`inline-radio`}>
                                                                                                        {
                                                                                                            item.purities.map((val, i) => (
                                                                                                                <Form.Check
                                                                                                                    ...
                                                                                                                    type={'radio'}
                                                                                                                    onChange={() => this.handlePurityChange(item.material_id, val.id)}
                                                                                                                    id={'p-' + val.id}
                                                                                                                />
                                                                                                            ))
                                                                                                        }
                                                                        ...
                                                                        <Row>
                                                                            <Col>
                                                                                <Form.Group className="mb-3">
                                                                                    ...
                                                                                    <InputGroup className={(this.state.weight_err ? 'is-invalid error_input' : '')}>
                                                                                        <Form.Control type="text" placeholder="Enter weight" value={this.state.weight} onChange={this.handleWeight} />
                                                                                        <InputGroup.Text>{sizeMaterial.materials[0].unit_name}</InputGroup.Text>
                                                                                    </InputGroup>
                                                                            ...
                                                                            <Col>
                                                                                <Form.Group className="mb-3">
                                                                                    ...
                                                                                    <Form.Control type="text" placeholder="Enter quantity" value={this.state.qty} onChange={this.handleQty} className={(this.state.qty_err ? 'is-invalid error_input' : '')} />
                                                                                    <Form.Control.Feedback type="invalid">
                                                                                        {this.state.qty_err}
                                                                                    </Form.Control.Feedback>
                                                                                </Form.Group>
                                                            ...
                                                            <div className='product-buttons'>
                                                                ...
                                                                <Button variant="primary" onClick={this.handleOrderNow}>ORDER NOW</Button>
                                                            </div>
                                                                            ...
                                                                            <ul className="authenticity_list">
                                                                                {
                                                                                    product.certificates.map((item, k) => (
                                                                                        <a href={item.website} target="_blank" key={k}><li><img src={item.logo} alt='' /></li></a>
                                                                                    ))
                                                                                }
                                                                            </ul>
                                                                        </>
                                                            ...
                                                            <a href="tel:+91 98744 45878">  <p className='mt-2 m-0 text-center reach-us'>Any Questions? Please feel free to reach us at: <span> +91 98744 45878 </span></p> </a>

                                                            {
                                                                review_data ?
                                                                    <>
                                                                        {
                                                                            review_data.total > 0 ?
                                                                                <div className="rating_sec bg-white rounded shadow-sm p-4 mb-4 clearfix graph-star-rating"><h5 className="mb-0 mb-4">Ratings and Reviews</h5><div className="graph-star-rating-header"><div className="star-rating"> <a href="#"><i className="icofont-ui-rating active"></i></a> <a href="#"><i className="icofont-ui-rating active"></i></a> <a href="#"><i className="icofont-ui-rating active"></i></a> <a href="#"><i className="icofont-ui-rating active"></i></a> <a href="#"><i className="icofont-ui-rating"></i></a> <b className="text-black ml-2">{review_data.total}</b></div><p className="text-black mb-4 mt-2">Rated {product.rating} out of 5</p></div><div className="graph-star-rating-body"><div className="rating-list"><div className="rating-list-left text-black"> 5 Star</div><div className="rating-list-center"><div className="progress"><div style={{ "width": `${review_data.total_5}%` }} aria-valuemax="5" aria-valuemin="0" aria-valuenow="5" role="progressbar" className="progress-bar bg-primary"> <span className="sr-only"></span></div></div></div><div className="rating-list-right text-black">{review_data.total_5}%</div></div><div className="rating-list"><div className="rating-list-left text-black"> 4 Star</div><div className="rating-list-center"><div className="progress"><div style={{ "width": `${review_data.total_4}%` }} aria-valuemax="5" aria-valuemin="0" aria-valuenow="5" role="progressbar" className="progress-bar bg-primary"> <span className="sr-only"></span></div></div></div><div className="rating-list-right text-black">{review_data.total_4}%</div></div><div className="rating-list"><div className="rating-list-left text-black"> 3 Star</div><div className="rating-list-center"><div className="progress"><div style={{ "width": `${review_data.total_3}%` }} aria-valuemax="5" aria-valuemin="0" aria-valuenow="5" role="progressbar" className="progress-bar bg-primary"> <span className="sr-only"></span></div></div></div><div className="rating-list-right text-black">{review_data.total_3}%</div></div><div className="rating-list"><div className="rating-list-left text-black"> 2 Star</div><div className="rating-list-center"><div className="progress"><div style={{ "width": `${review_data.total_2}%` }} aria-valuemax="5" aria-valuemin="0" aria-valuenow="5" role="progressbar" className="progress-bar bg-primary"> <span className="sr-only"></span></div></div></div><div className="rating-list-right text-black">{review_data.total_5}%</div></div></div></div>
                                                                                : null
                                                                        }

                                                                        <div className="rating_sec bg-white rounded shadow-sm p-4 mb-4 restaurant-detailed-ratings-and-reviews"><h5 className="mb-1">
                                                                            {
                                                                                review_data.total > 0 ?
                                                                        ...
                                                                        </h5>
                                                                            {
                                                                                review_data.items.map((item, index) => (
                                                                                    <React.Fragment key={index}>
                                                                                        <div className="reviews-members pt-4 pb-4"><div className="media"> <a href="#"><img src={item.user_image} className="mr-3 rounded-pill" /></a><div className="media-body"><div className="reviews-members-header"> <span className="star-rating float-right"> <a href="#"><i className="icofont-ui-rating active"></i></a> <a href="#"><i className="icofont-ui-rating active"></i></a> <a href="#"><i className="icofont-ui-rating active"></i></a> <a href="#"><i className="icofont-ui-rating active"></i></a> <a href="#"><i className="icofont-ui-rating"></i></a> </span><h6 className="mb-1"><a className="text-black" href="#">{item.user_name}</a></h6><p className="text-gray">{item.created_at}</p></div><div className="reviews-members-body"><p>{item.review}</p></div></div></div></div>
                                                                                        {
                                                    ...
                                                    <Col xs={12} md={5}>
                                                        ...
                                                        <div className='righ-side-wrapper'>
                                                            ...
                                                            <div className='price-breakup'>
                                                                ...
                                                                <div className='breakup-content'>
                                                                    ...
                                                                    <div className='breakup-item'>
                                                                        <span>GST</span>
                                                                        <span>{displayAmount(sizeMaterial.total_gst)}</span>
                                                                    </div>
                                                                    <hr style={{ margin: '8px 0' }} />
                                                                    <div className='breakup-item'>
                                                                        <span className='total'>Total </span>  <span className='total'>{displayAmount(sizeMaterial.sale_price * cartWeight)} </span>
                                                                        ...
                                                                        <Accordion.Body>
                                                                            <div className='p-size'>

                                                                                {
                                                                                    product.type != "material" ?
                                                                                        <span>Select Size
                                                                                            <Form.Select onChange={(e) => this.handleSizeChange(e.target.value)}>
                                                                                                {
                                                                                                    product.size_materials.map((item, index) => (
                                                                                                        <option value={index} key={index}>{item.size_name}</option>
                                                                                                    ))
                                                                                                }
                                                                                            </Form.Select>
                                                                                        </span>
                                                                                ...
                                                                                {
                                                                                    sizeMaterial.materials.map((item, key) => (
                                                                                        <span className='gold-type' key={key}>{sizeMaterial.materials.length > 1 ? item.material_name : "Purity"}
                                                                                            <Form.Select onChange={(e) => this.handlePurityChange(item.material_id, e.target.value)} value={this.getSelectedVal(item.purities)}>
                                                                                                {
                                                                                                    item.purities.map((val, i) => (
                                                        ...
                                                        <div className='product-details-items mt-2'>
                                                            <Accordion alwaysOpen>
                                                                <Accordion.Item eventKey="0">
                                                                    <Accordion.Header><div className='header'>
                                                                        <h2 style={{ marginBottom: '0px' }}>Product Details</h2>
