...
import { ListItemText, Checkbox, Link, Box, TextField, Grid, Button, CircularProgress, Pagination, IconButton, Stack, FormControl, InputLabel, Select, MenuItem } from '@mui/material';
import LoginForm from 'forms/SuperAdmin/LoginForm';
import { bindActionCreators } from 'redux';
import { gridSpacing } from 'store/constant';
import MainCard from 'ui-component/cards/MainCard';
import withRouter from 'src/helpers/withRouter';
import { materialPriceRawList, materialPriceDelete } from 'actions/superadmin/materialPrice.actions';
...
import { materialList, materialCreate } from 'actions/superadmin/material.actions';
...
import {RESET_MATERIAL, SET_MATERIAL} from 'actionTypes/superadmin/material.types';
    ...
    if(res.data.success){
      this.setState({
        ...
        if(this.state.material_id && !res.data.data.items.length){
          let material = _.filter(this.state.materialList, { id: this.state.material_id });
          this.props.dispatch({
            type: SET_MATERIAL,
            payload: material[0]
          });
  ...
  componentDidUpdate(previousProps, previousState) {
    ...
    if(this.state.m_actionCalled){
      if(this.state.m_createSuccess){
        this.props.enqueueSnackbar(this.state.m_successMessage, {variant: 'success'});
        this.setState({
          create_material_price: true,
          is_new_material: false,
          material_form: {
        ...
        this.props.enqueueSnackbar(this.state.m_errorMessage, {variant: 'error'});
      }
    ...
    if(this.state.mp_actionCalled){
      if(this.state.mp_createSuccess){
        this.props.enqueueSnackbar(this.state.mp_successMessage, {variant: 'success'});
        this.props.dispatch({
          type: RESET_MATERIAL_PRICE
        });
        this.setState({
        ...
        this.props.enqueueSnackbar(this.state.mp_errorMessage, {variant: 'error'});
      }
      ...
      <MainCard title="Price List">
        <Box sx={{ flexGrow: 1, m: 0.5 }} className='ratn-dialog-inner'>
            <Grid container spacing={2} className='tax-input loans_view p_view'>
              <Grid item xs={this.state.is_new_material ? 3 : 6} className='create-input'>
              ...
              <Grid item xs={this.state.is_new_material ? 3 : 6} className='create-input'>
                <Autocomplete
                  ...
                  filterOptions={(options, params) => {
                    ...
                    if (inputValue !== '' && !isExisting && hasPermission(this.state.permissions, 'material', 'add') ) {
                      filtered.push({
                        inputValue,
                        name: `Add "${inputValue}"`,
                      });
              ...
              </Grid>
              {
                ...
                <Grid item xs={4} className='create-input'>
                  <FormControl fullWidth error={this.state.material_form_errors.purities}>
                    ...
                    >
                      {this.state.purities.map((item) => (
                        <MenuItem key={item.id} value={item.id} className='multi-select'>
                          <Checkbox checked={(this.state.material_form.purities && this.state.material_form.purities.indexOf(item.id) > -1) ? true : false} />
                ...
                <Grid item xs={2} className='create-input'>
                  <FormControl fullWidth error={this.state.material_form_errors.unit_id}>
            ...
            </Grid>
            {
              ...
              <Grid item xs={12}>
                <Stack spacing={1} direction="row" className='modal-button-area ratn-footer-buttons' sx={{mb: 2}}>
                  <Button variant="contained" className='conf-button' onClick={this.materialFormSubmit}>Submit</Button>
                  <Button variant="outlined" onClick={this.materialFormCancel}>Cancel</Button>
                </Stack>
              </Grid>
              : null
            }
              ...
              !this.state.items.length ?
                <Grid container justifyContent="center">
                  <span className='data-found'>
                    {
                      !this.state.category_id ? "Please select category." : "No data found."
                    }
                  </span>
                </Grid>
                :
                <>
                  {
                    this.state.items.map((item, index) => (
                      <React.Fragment key={index}>
                        ...
                        <TableContainer component={Paper} className="table_container ratn-table-wrapper">
                          <Table sx={{ minWidth: 500 }} className="price_list_table" border="1">
                            <TableBody>
                              <TableRow>
                                <TableCell padding="none" className="th_header text-white bg-black" style={ {fontWeight : '600'}} sx={{ width: 230 }}>
                                  Material
                                </TableCell>
                                <TableCell padding="none" className="th_header text-white bg-black" style={ {fontWeight : '600'}} sx={{ width: 110 }}>
                                  Purity
                                </TableCell>
                                {
                                  item.purities.map((item, index) => (
                                    <TableCell key={index} padding="none" className="th_header text-white bg-black" style={ {fontWeight : '600'}}>
                                      {item.purity_name}
                                    </TableCell>
                                  ))
                                }
                                <TableCell padding="none" className="th_header th_header_action price-th text-white bg-black" style={ {fontWeight : '600'}}>
                                  Action
                                </TableCell>
                              </TableRow>
                              <TableRow>
                                <TableCell padding="none" className="icon-btn-height"  rowSpan={7}>
                                  <IconButton
                                ...
                                <TableCell padding="none" className="icon-btn-height">
                                  {/*<IconButton
                                    aria-label="expand row"
                                    size="small"
                                    onClick={() => this.setOpen(item.id)}
                                ...
                                </TableCell>
                                {
                                  item.purities.map((p_item, p_index) => (
                                    <TableCell key={p_index} padding="none" className='cell_padding'>
                                      {p_item.display_price}
                                    </TableCell>
                                  ))
                                }
                                <TableCell rowSpan={7} padding="none" className="cell_padding">
                                  <Stack direction="row" alignItems="center" spacing={2}>
                                    {
                                      hasPermission(this.state.permissions, 'price_list', 'edit') ?
                                      <IconButton color="primary" component="label" onClick={() => this.handelEdit(item.id)}>
                                        <EditIcon />
                                      </IconButton>
                                      : null
                                    }
                                    {
                                      hasPermission(this.state.permissions, 'price_list', 'delete') ?
                                      <IconButton color="error" component="label" onClick={() => this.handleDelete(item.id)}>
                                        <DeleteIcon />
                                      </IconButton>
                                      : null
                                    }
                                ...
                                <>
                                  <TableRow>
                                    ...
                                    </TableCell>
                                    {
                                      item.purities.map((p_item, p_index) => (
                                        <TableCell key={p_index} padding="none" className='cell_padding'>
                                          {p_item.increase}% | {displayAmount(p_item.mrp)}
                                        </TableCell>
                                      ))
                                    }
                                  </TableRow>
...

        {
          ...
          <MaterialPriceForm material={this.state.material_data} handleCancel={this.handleCancelPriceCreate} />
          : null
        }
...
export default withSnackbar(withRouter(connect(mapStateToProps, mapDispatchToProps)(MaterialPricePage)));
