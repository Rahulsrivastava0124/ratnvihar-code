...
import {Box, TextField, Button, Grid, Link,Card,CardActions, CardContent,Typography, TextareaAutosize, Stack,  Select, MenuItem, InputLabel, FormControl, FormHelperText, InputAdornment, Autocomplete  } from '@mui/material';
import { bindActionCreators } from 'redux';
import { gridSpacing } from 'store/constant';
import MainCard from 'ui-component/cards/MainCard';
import withRouter from 'src/helpers/withRouter';
import { supplierList } from 'actions/superadmin/supplier.actions';
import { paymentList, walletHistoryList, paymentStatusChange, paymentStore } from 'actions/superadmin/payment.actions';
...
import {isEmpty, isSuperAdmin, isAdmin, isDistributor, isSalesExecutive} from 'src/helpers/helper';
...
class WalletPage extends Component {
    ...
    render() {    
        const {formValues, formErros, balance_by_mode, paymentFormValues, paymentFormErros, profile} = this.state;
        let userList = [], userListLabel = '';
        ...
        <MainCard title="Wallet History"  secondary=''>
            {
                ...
                <Card className='dashboard_card'>
                    <CardContent className='dashboard_card_content bg-color-1' sx = {{ display: "flex", justifyContent: "space-between" }} onClick={() => this.handleCardClick('cash')}>
                        <Typography sx={{ fontSize: 14, margin: 0 }} color="text.secondary" gutterBottom component="span">
                        <h1>Cash</h1>
                        <h2>
                            {balance_by_mode.cash}
                        </h2>
                        </Typography>
                    ...
                    <CardContent className='dashboard_card_content bg-color-2' sx = {{ display: "flex", justifyContent: "space-between" }} onClick={() => this.handleCardClick('cheque')}>
                        <Typography sx={{ fontSize: 14, margin: 0 }} color="text.secondary" gutterBottom component="span">
                        <h1>Cheque</h1>
                        <h2>
                            {balance_by_mode.cheque}
                        </h2>
                        </Typography>
                        <div className="card-icon">
                        <AccountBalanceWalletIcon />
                    ...
                    <CardContent className='dashboard_card_content bg-color-3' sx = {{ display: "flex", justifyContent: "space-between" }} onClick={() => this.handleCardClick('imps_neft')}>
                        <Typography sx={{ fontSize: 14, margin: 0 }} color="text.secondary" gutterBottom component="span">
                        <h1>NEFT/IMPS/UPI</h1>
                        <h2>
                            {balance_by_mode.imps_neft}
                        </h2>
                        </Typography>
                        <div className="card-icon">
                        <AccountBalanceWalletIcon />
                    ...
                    <CardContent className='dashboard_card_content bg-color-4' sx = {{ display: "flex", justifyContent: "space-between" }} onClick={() => this.handleCardClick('online')}>
                        <Typography sx={{ fontSize: 14, margin: 0 }} color="text.secondary" gutterBottom component="span">
                        <h1>Online</h1>
                        <h2>
                            {balance_by_mode.online}
                        </h2>
                        </Typography>
                        <div className="card-icon">
                        <AccountBalanceWalletIcon />
            ...
            <Grid container spacing={gridSpacing} columnSpacing={{ xs: 1, md: 3 }} sx={{mb: 2}} rowSpacing={{ xs: 2}} className='mob_responsive_input'>
                <Grid item xs={6} md={2} className='create-input'>
                    <FormControl fullWidth>
                        ...
                        label="Status"
                        onChange={(e) => this.updateQueryParams(e.target.value, 'payment_mode')}
                ...
                <Grid item xs={6} md={1} className='create-input button-right'>
                    <Button variant="contained" className='search-btn ml-10' onClick={this.handlePayNow}>Pay</Button>
                </Grid>
            </Grid>
                        ...
                        <Grid container spacing={2}>
                            {
                                ...
                                <>
                                {
                                    this.state.actionRow && this.state.actionRow.payment_mode == "Cheque" ?
                ...
                <DialogContentText></DialogContentText>
                    <Box sx={{ flexGrow: 1, m: 0.5 }}>
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={12} md={3} className='p-invoice-date create-input'>
                                <LocalizationProvider dateAdapter={AdapterDayjs}>
                                    <DatePicker
                                        ...
                                        renderInput={(params) => <TextField {...params} fullWidth error={paymentFormErros.payment_date} />}
                                    />
                                </LocalizationProvider>
                            </Grid>
                            <Grid item xs={12} md={3} className='create-input'>
                              <FormControl fullWidth error={paymentFormErros.payment_role}>
                                ...
                                <Select
                                    ...
                                    fullWidth
                                    label="Role"
                                    onChange={(event) => this.handlePaymentRole(event.target.value)}
                                >
                                    {
                            ...
                            <Grid item xs={12} md={3} className='create-input'>
                              <FormControl fullWidth error={paymentFormErros.user_id}>
                                ...
                                <Autocomplete
                                    ...
                                    options={userList}
                                    autoHighlight
                                    getOptionLabel={(option) => (paymentFormValues.payment_role == "retailer" ? option.company_name : option.name)}
                                    renderOption={(props, option) => <li {...props} key={option.id}>{paymentFormValues.payment_role == "retailer" ? option.company_name : option.name}</li>}
                                    renderInput={(params) => (
                                        <TextField
                                            {...params}
                                            label={userListLabel}
                                    ...
                                    onChange={(event, newValue) => {
                                        this.updateFormValue(newValue ? newValue.id : '', 'user_id')
                                    }}
                                />
                              </FormControl>
                            </Grid>
                            ...
                            {
                                (paymentFormValues.payment_mode == "imps_neft" || paymentFormValues.payment_mode == "upi") ?
                                <Grid item md={6} xs={12} className='create-input'>
                                    <TextField  
                                        label="Transaction #" 
                                        variant="outlined"
                            ...
                            <Grid item xs={12} >
                              <Stack spacing={1} direction="row" justifyContent="flex-end">
                                  <Button variant="contained" type="button" disabled={this.state.processing} onClick={this.handlePaymentSubmit}>
                                      {
                                          this.state.processing ? "Processing" : "Submit"
                                      }
                                  </Button>
                                  <Button variant="outlined" onClick={this.handlePaymentDialogClose}>Cancel</Button>
                              </Stack>
                            </Grid>
                        </Grid>
                    </Box>
...
const mapDispatchToProps = dispatch => {
  return {
    dispatch,
    actions: bindActionCreators({walletHistoryList, adminList, supplierList, distributorList, paymentStore, retailerList}, dispatch)
  }
};  


export default withSnackbar(withRouter(connect(mapStateToProps, mapDispatchToProps)(WalletPage)));
