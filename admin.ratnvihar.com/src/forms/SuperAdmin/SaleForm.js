...
import { Box, TextField, Button, Grid, Link, TextareaAutosize, Stack, Select, MenuItem, InputLabel, FormControl, FormControlLabel, Checkbox, FormHelperText, Autocomplete, FormLabel, ImageList, ImageListItem, InputAdornment, IconButton, RadioGroup, Radio, Collapse, Alert } from '@mui/material';
import { ContactPageSharp } from '@mui/icons-material';
import { calculateAdminProductPrice, priceFormat, getValuesFromKey, isEmpty, toBase64, calculateGST, displayAmount, weightFormat, isSuperAdmin, isDistributor, isAdmin, isSalesExecutive } from 'src/helpers/helper';
import { bindActionCreators } from 'redux';
import { salesStore, salesUpdate, salesViewRaw, saleReturn } from 'actions/superadmin/sales.actions';
import { stocksProductList, stocksProducDetails } from 'actions/superadmin/stocks.actions';
...
import { cartDelete, cartListRaw, cartList } from 'actions/superadmin/cart.actions';
...
class SaleForm extends React.Component {
    ...
    initializeFormData = () => {
        ...
        let return_products = [], discount_per_product = 0, total_products = 0;
        for (let i = 0; i < formValues.products.length; i++) {
            ...
            if (formValues.products[i].product_type == "material") {
                ...
                total_products += parseFloat(formValues.products[i].materials[0].quantity);
            } else {
                total_products++;
            }
        }
        if (parseFloat(formValues.discount) > 0) {
            ...
            discount_per_product = priceFormat(parseFloat(formValues.discount) / total_products);
        }
        this.setState({
            ...
            if (res.data.success) {
                ...
                this.setState({
                    formValues: {
                        ...
                        paid_amount: res.data.data.paid_amount ? res.data.data.paid_amount : '',
    ...
    loadCart = async () => {
        let response = await cartListRaw({ from_order_price: this.props.query.get('from_order_price'), order_id: this.props.query.get('order_id') });
        if (response.data.success) {
            ...
            for (let i = 0; i < cartList.length; i++) {
                ...
                for (let item of cart.materials) {
                    ...
                    let index = _.findIndex(unique_materials, (p) => p.material_id == item.material_id);
                ...
                let result2 = calculateGST(cart.tax_info, (parseFloat(cart.total_amount)), this.state.user_gst_no);
                ...
                let total = priceFormat(cart.total_amount + cgst_tax + sgst_tax + igst_tax);
                ...
                products.push({
                    ...
                    max_making_charge_discount_percent: cart.max_making_charge_discount_percent,
    ...
    componentDidUpdate(prevProps) {
        if (this.props.formData != prevProps.formData) {
            ...
            if (this.props.query.get('sale_on_approval') != prevProps.query.get('sale_on_approval')) {
                this.loadSaleOnApproval();
            }
        }
            ...
            if (this.state.isCreateFrom) {
                if (this.state.createSuccess) {
                    ...
                    if (this.state.isAssign) {
                        this.props.navigate(getUserDashboardRoute(getRoleName(this.state.auth)) + '/transfer');
                    } else if (this.state.isOnApprove) {
                        this.props.navigate(getUserDashboardRoute(getRoleName(this.state.auth)) + '/sale-on-approve');
                    } else {
                        this.props.navigate(getUserDashboardRoute(getRoleName(this.state.auth)) + '/sales');
                    }
                } else {
                    this.setState({
                        submitting: false
        ...
        if (prevProps.order != this.props.order && this.props.order) {
            ...
            setTimeout(() => {
                this.setState({
                    ...
                    if(this.props.order.is_customer && this.props.order.paid_amount > 0){
                        advance_amount = this.props.order.paid_amount;
                    }
                    this.handleAdminChange('', this.props.order.user_details.id, advance_amount)
                    //this.handleCalculateMainPrice();
                })
            }, 1000);
        }
    ...
    handleAdminChange = (event, val, advnc_amt) => {
        ...
        let user_gst_no = '', advance_amount = 0;
        if (m.length) {
            user_gst_no = m[0].gst;
            if (/*(this.isSalesExecutive || this.isSalesExecutive) &&*/ !this.state.isAssign) {
        ...
        this.setState({
            ...
            if ((this.isSuperAdmin || (this.isAdmin && this.state.profile && this.state.profile.own)) && m.length && m[0].own) {
    ...
    calculateProductPrice = () => {
        ...
        for (let x = 0; x < products.length; x++) {
            let total_price = 0, total_price_with_discount = 0, making_charge = 0, total_discount = 0, total_quantity = 0;
            for (let i = 0; i < products[x].materials.length; i++) {
                let discount_percent = products[x].materials[i].discount_percent ? parseFloat(products[x].materials[i].discount_percent) : 0;
                discount_percent = !isAssign ? discount_percent : 0;
                let disPerGramPrice = priceFormat(parseFloat(products[x].materials[i].per_gram_price) - parseFloat(products[x].materials[i].per_gram_price * parseFloat(discount_percent) / 100));
                let thisPrice = priceFormat(parseFloat(products[x].materials[i].per_gram_price) * parseFloat(products[x].materials[i].total_gram));
                total_price += thisPrice;
                total_price_with_discount += priceFormat(parseFloat(disPerGramPrice) * parseFloat(products[x].materials[i].total_gram));
                products[x].materials[i].amount = thisPrice;
                products[x].materials[i].discount_amount = priceFormat(thisPrice - priceFormat(parseFloat(disPerGramPrice) * parseFloat(products[x].materials[i].total_gram)));
                ...
                total_discount += priceFormat(thisPrice - priceFormat(parseFloat(disPerGramPrice) * parseFloat(products[x].materials[i].total_gram)));//parseFloat(products[x].materials[i].rate * discount_percent / 100);
            }

            let isMaterial = products[x].product_type == "material" ? true : false;
            if (products[x].sub_cat_making_charge_type == "per_piece") {
                making_charge = priceFormat(parseFloat(products[x].sub_cat_making_charge));
            } else if (products[x].sub_cat_making_charge_type == "per_gram") {
                making_charge = priceFormat(parseFloat(products[x].total_weight) * parseFloat(parseFloat(products[x].sub_cat_making_charge)));
            }

            let discount_amount = !isAssign ? priceFormat(making_charge * parseFloat(products[x].making_charge_discount_percent) / 100) : 0;
            let total_making_charge = priceFormat(making_charge - discount_amount);
            total_discount += discount_amount;

            let result2 = !isAssign ? calculateGST(products[x].tax_info, (parseFloat(total_price_with_discount) + parseFloat(total_making_charge)), this.state.user_gst_no) : null;
            let cgst_tax = result2 ? result2.cgst : 0;
            let sgst_tax = result2 ? result2.sgst : 0;
            let igst_tax = result2 ? result2.igst : 0;
            let total_tax = priceFormat(cgst_tax + sgst_tax + igst_tax);
            let total_amount = priceFormat(total_making_charge + total_price_with_discount);
            ...
            products[x].sub_price = priceFormat((parseFloat(total_price) + parseFloat(making_charge)));
    ...
    handleCalculateMainPrice = () => {
        let formValues = this.state.formValues;
        let product_discount = 0, total_tag_price = 0;
        ...
        for (let i = 0; i < formValues.products.length; i++) {
            taxable_amount += (parseFloat(formValues.products[i].total) - parseFloat(formValues.products[i].total_tax));
        ...
        let advance_amount = formValues.advance_amount ? parseFloat(formValues.advance_amount) : 0;
        if (this.state.isCreateFrom) {
        ...
        if (formValues.pay_from_advance) {
            due_amount = advance_amount > due_amount ? 0 : priceFormat(due_amount - advance_amount, true);
        }
        ...
        formValues.total_payable = this.state.isCreateFrom ? total_payable : formValues.total_payable;
    ...
    handleDeleteConfirm = async () => {
        ...
        let response = await cartDelete(products[this.state.deletingIndex].id, true);
        if (response.data.success) {
    ...
    handleSubmit = async (isApproval) => {
        ...
        if (!hasErr && formValues.products.length) {
            ...
            let data = { ...this.state.formValues, on_approval: isApproval, on_approval_id: this.props.query.get('sale_on_approval') };
            data.order_id = this.state.order ? this.state.order.id : 0;
            data.order_from_customer = this.state.order ? this.state.order.is_customer : false;
            if (this.state.order && 'is_retailer' in this.state.order && this.state.order.is_retailer) {
    ...
    formValidate = (isApproval) => {
        ...
        if (parseFloat(formValues.due_amount) > 0) {
            ...
            if (!this.state.isAssign && isEmpty(formValues.settlement_date) && !isApproval) {
    ...
    handleMaterialDisc = (event, productKey, materialKey) => {
        ...
        formValues.products[productKey].materials[materialKey].discount_percent = value;
        this.setState({
            formValues: formValues
        }, () => {
            this.calculateProductPrice();
    ...
    handleCommonDis = (event, index) => {
        ...
        for (let i = 0; i < formValues.products.length; i++) {
            for (let x = 0; x < formValues.products[i].materials.length; x++) {
                if (unique_materials[index].material_id == formValues.products[i].materials[x].material_id) {
        ...
        for (let i = 0; i < formValues.products.length; i++) {
            if (!vl) {
                ...
                if (formValues.products[i].max_making_charge_discount_percent > 0) {
                    formValues.products[i].making_charge_discount_percent = formValues.products[i].max_making_charge_discount_percent >= parseFloat(vl) ? vl : formValues.products[i].max_making_charge_discount_percent;
                }
            }
        }
        this.setState({
    ...
    getUserList = () => {
        ...
        if (this.isSuperAdmin) {
            ...
            if (this.state.isAssign) {
                userList = this.state.distributorList.concat(this.state.salesExecutiveList);
            } else {
                userList = this.state.retailerList;
            }
        }
    ...
    handleTransfer = (user_id) => {
        ...
        this.setState({
            isAssign: true,
            formValues: { ...this.state.formValues, user_id: user_id, invoice_number }
        }, () => {
            this.calculateProductPrice();
        })
    }
    ...
    handleBackAssign = () => {
        ...
        this.setState({
            isAssign: false,
            formValues: { ...this.state.formValues, user_id: '', invoice_number: invoice_number }
        }, () => {
            this.calculateProductPrice();
        })
    }
                ...
                if (return_products[i].is_return == true) {
                    ...
                    if (returnP.length) {
                        if (returnP[0].category_id != product.category_id) {
                            return this.props.enqueueSnackbar("You can't return different category product in one invoice.", { variant: 'error' });
                        }
                    }
                }
            }
    ...
    handleReturn = () => {
        ...
        if (!res.isReturn) {
            return this.props.enqueueSnackbar("Please select return product.", { variant: 'error' });
        }
    ...
    handleReturnConfirm = async () => {
        ...
        let res = await saleReturn(this.state.formData.id, {
            ...
            return_amount_from_wallet: (priceFormat(parseFloat(this.state.return_amount) - parseFloat(this.state.formValues.due_amount)))
        });
        if (res.data.success) {
    ...
    calculateReturnAmount = () => {
        ...
        for (let i = 0; i < return_products.length; i++) {
            if (return_products[i].is_return) {
                if (formValues.products[i].product_type == "material") {
                    let discount = priceFormat(parseFloat(formValues.products[i].materials[0].rate) * parseFloat(formValues.products[i].materials[0].discount_percent) / 100);
                    let rate = priceFormat(parseFloat(formValues.products[i].materials[0].rate) - discount);
                    let thisAmt = priceFormat(parseFloat(formValues.products[i].materials[0].return_weight) * rate);
                    //thisAmt = priceFormat(thisAmt - (parseFloat(formValues.products[i].materials[0].return_weight) * discount_per_product));
                    thisAmt = priceFormat(thisAmt - (parseFloat(formValues.products[i].materials[0].return_qty) * discount_per_product));
                    let tax = priceFormat(parseFloat(formValues.products[i].total_tax) / parseFloat(formValues.products[i].materials[0].return_qty));
                    console.log(thisAmt, tax, discount_per_product);
                    thisAmt = priceFormat(thisAmt + tax);
                    let thisReturnCharge = formValues.have_return_charge ? (parseFloat(formValues.products[i].return_charge_percent) > 0 ? priceFormat(thisAmt * parseFloat(formValues.products[i].return_charge_percent) / 100) : 0) : 0;
                    ...
                    let thisReturnCharge = formValues.have_return_charge ? (parseFloat(formValues.products[i].return_charge_percent) > 0 ? (thisAmt * parseFloat(formValues.products[i].return_charge_percent) / 100) : 0) : 0;
                    formValues.products[i].return_amount = thisAmt;
                    formValues.products[i].return_charge = thisReturnCharge;
                    return_amount += (thisAmt - thisReturnCharge);
                    return_charge += thisReturnCharge;
        ...
        let returnDis = 0;
        let didNotReturned = 0, totalReturnP = 0;
        for (let i = 0; i < this.state.return_products.length; i++) {
            let product = _.filter(this.state.formValues.products, { id: this.state.return_products[i].id });
    ...
    hasReturn = () => {
        let isReturn = 0, will_return_charge_apply = false;
        for (let i = 0; i < this.state.return_products.length; i++) {
            let product = _.filter(this.state.formValues.products, { id: this.state.return_products[i].id });
            if (product.length && product[0].is_return == true) {
                continue;
        ...
        return { isReturn: isReturn, will_return_charge_apply: will_return_charge_apply }
    }
    ...
    render() {
        const { formValues, formErros, productFormValues, productFormErros, materialFormErros, submitting, order, actionProductIndex } = this.state;
        ...
        let user = formValues.user_id ? _.filter(userList, { id: formValues.user_id }) : [];
        ...
        if (parseFloat(formValues.due_amount) == 0 && parseFloat(formValues.total_payable) == parseFloat(formValues.paid_amount)) {
            let didNotReturned = 0, totalReturnP = 0;
            for (let i = 0; i < this.state.return_products.length; i++) {
                let product = _.filter(formValues.products, { id: this.state.return_products[i].id });
                if (product.length && product[0].is_return == true) {
                    continue;
                }
                if (this.state.return_products[i].is_return) {
            ...
            if (totalReturnP == 1 && didNotReturned == 0) {
                ...
                return_from_wallet = priceFormat(return_from_wallet - this.state.return_charge)
            } else if (totalReturnP == 1 && didNotReturned > 0) {
                return_from_wallet = parseFloat(this.state.return_amount);
            }
        } else {
            if ((this.state.formValues.due_amount && parseFloat(this.state.return_amount) > parseFloat(this.state.formValues.due_amount))) {
                return_from_wallet = priceFormat(parseFloat(this.state.return_amount) - parseFloat(this.state.formValues.due_amount));
            }
        }


        return (
            <Box sx={{ flexGrow: 1, m: 0.5 }} className='ratn-dialog-inner sale_create_page'>
                <Grid container spacing={2} columnSpacing={{ xs: 1, sm: 2, md: 2 }} className='tax-input loans_view p_view'>
                    ...
                    <Grid container spacing={2} columnSpacing={{ xs: 1, sm: 2, md: 2 }}>
                            ...
                            this.state.isAssign ?
                                <Grid item xs={12} md={2} className='create-input'>
                                    <FormControl fullWidth>
                                        <Button variant="contained" size="small" onClick={this.handleBackAssign} startIcon={<ArrowBackIcon />}>Back</Button>
                                    </FormControl>
                                </Grid>
                                : null
                        }
                            ...
                            <FormControl fullWidth error={formErros.user_id}>
                                {
                                    order && order.is_customer ?
                                        ...
                                        <Autocomplete
                                            ...
                                            value={userIdValue}
                                            autoHighlight
                                            getOptionLabel={(option) => (this.state.isAssign ? option.name : option.company_name)}
                                            renderOption={(props, option) => <li {...props} key={option.id}>{this.state.isAssign ? option.name : option.company_name}</li>}
                                            renderInput={(params) => (
                                                <TextField
                                                    {...params}
                                                    label={this.state.isAssign ? "Transfer To" : "Company Name"}
                                            ...
                                            onChange={(event, newValue) => {
                                                this.handleAdminChange(event, newValue ? newValue.id : '')
                                            }}
                                            disabled={!this.state.isCreateFrom || (!isEmpty(this.props.query.get('sale_on_approval')) ? true : false) || (order ? true : false)}
                                        />
                                }
                            </FormControl>
                        </Grid>
                        ...
                        <Grid item xs={!formValues.user_id ? 6 : 6} md={!formValues.user_id ? 3 : 2} className='create-input p-invoice-date'>
                            <LocalizationProvider dateAdapter={AdapterDayjs}>
                                <DatePicker
                                    ...
                                    renderInput={(params) => <TextField {...params} error={formErros.invoice_date} className="non_disable_text" />}
                                />
                ...
                <Grid container spacing={2} columnSpacing={{ xs: 1, sm: 2, md: 2 }} className='tax-input loans_view'>
                    <Grid item xs={12} md={12} className=' create-input p-add-product border-radius-0'>
                        ...
                        <TableContainer component={Paper}>
                            <Table sx={{ minWidth: 650 }} aria-label="simple table" className='ratn-table-product-wrapper sale_form_table'>
                                        ...
                                        formValues.products.map((item, index) => (
                                            <React.Fragment key={index}>
                                                <TableRow className='product_details'>
                                                    ...
                                                    <TableCell colSpan={7}>{item.certificate_no}</TableCell>
                                                    {
                                                        this.state.isCreateFrom ?
                                                            <TableCell rowSpan={2} className="action_column" style={{ textAlign: 'center' }}>
                                                                {/*<IconButton className='del-icon' color="error" component="label"  onClick={() => this.handleProductDelete(index)}>
                                                            <CloseIcon />
                                                            </IconButton> */}
                                                                <Button variant="contained" className='sale-cross-icon' onClick={() => this.handleProductDelete(index)}> X </Button>
                                                            </TableCell>
                                                            : null
                                                    }
                                                </TableRow>
                                                <TableRow className='material_details'>
                                                            ...
                                                            item.materials.map((m, key) => (
                                                                <div className='products-data-container' key={key}>
                                                                    <div className='products-data-row'>
                                                                        <div className='products-data' key={key} style={{ position: "relative" }}>{m.material_name} &nbsp;({m.purity}) &nbsp; &nbsp;{m.weight} &nbsp; &nbsp;{m.unit_name} &nbsp; &nbsp; x &nbsp; &nbsp; {m.rate} </div>
                                                                        <div className='products-amount'> = &nbsp; &nbsp;{m.amount}</div>
                                                                    </div>
                                                                </div>
                                                            ))
                                                        }
                                                    ...
                                                    <TableCell>
                                                        {
                                                            item.materials.map((m, key) => (
                                                                <div className='sale-discount-wrapper' key={key}>
                                                                    <>
                                                                        {
                                                                            m.max_discount_percent > 0 && !this.state.isAssign ?
                                                                                <>
                                                                                    Dis@ <div className='sale-discount'>
                                                                                        <input
                                                                                            type="text" value={m.discount_percent}
                                                                                            onChange={(event) => this.handleMaterialDisc(event, index, key)} className="custom_input" max={m.max_discount_percent} disabled={(isReturn) ? "disabled" : ""} /><div className='sale-discount-inner'> %</div></div> {m.mrp}
                                                                                </>
                                                                                :
                                                                                " - "
                                                                        }
                                                    ...
                                                    <TableCell>
                                                        {item.making_charge}{item.max_making_charge_discount_percent > 0 && !this.state.isAssign ? <>@ <span style={{ position: 'relative' }}>

                                                            <input
                                                                type="text" value={item.making_charge_discount_percent}
                                                                onChange={(event) => this.handleMakingDiscount(event, index)} className="custom_input" max={item.max_making_charge_discount_percent} disabled={(isReturn) ? "disabled" : ""} /><span style={{ position: 'absolute', right: '1px', top: "0px" }}> %</span></span> &nbsp;{priceFormat(item.making_charge - item.making_charge_discount_amount)}
                                                        </> : null}
                                                    </TableCell>
                                                    <TableCell>
                                                        {item.sub_price}
                                            ...
                                            <TableRow>
                                                ...
                                                <TableCell colSpan="4">
                                                    <div className='unique-wrapper'>
                                                        <div className='unique-wrapper-row'>
                                                            {
                                                                this.state.unique_materials.map((item, index) => (
                                                                    <React.Fragment key={index}>

                                                                        <div className='unique_materials'>
                                                                            ...
                                                                            <span style={{ position: 'relative' }}><input
                                                                                ...
                                                                                style={{ width: '100%', height: '40px', padding: '5px 8px' }}
                                                                                max={item.max_discount}
                                                                            /><span style={{ position: 'absolute', right: '5px', top: "0px" }}> %</span></span>
                                                                        </div>
                                                                    </React.Fragment>
                                                                ))
                                                            }
                                                ...
                                                */}
                                                        ...
                                                        this.haveMakingComonDis() ?
                                                            <>
                                                                ...
                                                                <span style={{ position: 'relative' }}><input
                                                                    ...
                                                                    onChange={(event) => this.handleCommonMakingDis(event)}
                                                                    className="custom_input"
                                                                    style={{ width: '90%', height: '40px', padding: '5px 8px' }}
                                                ...
                                                <TableCell sx={{ verticalAlign: "top" }} >
                                                    <b className='mob-hide'>
                                                        Tax<br />
                                                        {displayAmount(formValues.total_tax)}
                                                    </b>
                                                </TableCell>
                    ...
                    <Grid item xs={12} md={8} style={{}}>
                        <Grid container spacing={2} className='mob_responsive_purchase_input'>
                            <ul className='sale_total'>
                                <li>Price <span>{priceFormat(formValues.total_tag_price)}</span></li>
                                <li>Dist <span>{priceFormat(formValues.product_discount)}</span></li>
                                <li>Tax <span>{displayAmount(formValues.total_tax)}</span></li>
                                <li>Total Amount <span>{displayAmount(formValues.total_amount)}</span></li>
                            </ul>
                        </Grid>
                    </Grid>
                    <Grid item xs={12} md={8} style={{}}>
                        <Grid container spacing={2} columnSpacing={{ xs: 1, sm: 2, md: 2 }} className='mob_responsive_purchase_input'>
                            {
                                (this.isSalesExecutive || this.isDistributor || this.isSuperAdmin || this.isAdmin) && !this.state.isAssign && !formValues.user_id && this.state.isCreateFrom && isEmpty(this.props.query.get('sale_on_approval')) ?
                                    <Grid item xs={12} className='create-input button-right' style={{ paddingTop: '0px', paddingBottom: '7px' }}>
                                        <Button variant="contained" size="small" onClick={this.handleTransfer}>Transfer</Button>
                                    </Grid>
                                    : null
                            }
                            ...
                            {
                                (this.isSalesExecutive || this.isDistributor) && this.state.isAssign ?
                                    <>
                                        {
                                            formValues.image_file ?
                                                <Grid item xs={12} md={2} className='create-input' style={{ position: 'relative' }}>
                                                    <DeleteIcon onClick={this.deleteImage} className="image_delete" style={{
                                                        position: 'absolute',
                                                        right: 0,
                                                        color: '#ff0000',
                                                        cursor: 'pointer'
                                                    }} />
                                                    <img src={this.getImageSrc(formValues.image_file)} id="logo-img" style={{ height: '100px', width: '100px' }} />
                                                </Grid>
                                                :
                                                <Grid item xs={12} md={2} className='create-input'>
                                                    <img src={noImage} id="logo-img1" style={{ height: '100px', width: '100px' }} />
                                                </Grid>
                                        }
                    ...
                    {
                        !this.state.isAssign ?
                            <Grid item xs={12} md={4} style={{ paddingRight: '16px', paddingTop: '8px' }}>
                                <Grid container spacing={2} columnSpacing={{ xs: 1, sm: 2, md: 2 }} className='mob_responsive_purchase_input_table'>
                                            ...
                                            <>
                                                {
                                                    formValues.cgst_tax > 0 ?
                                                        <Grid item xs={12} md={12} className="pt-5">
                                                            <Grid container columnSpacing={{ xs: 1, sm: 2, md: 2 }} spacing={2} className="display_center">
                                                ...
                                                }
                                                {
                                                    formValues.sgst_tax > 0 ?
                                                        <Grid item xs={12} md={12} className="pt-5">
                                                            <Grid container spacing={2} columnSpacing={{ xs: 1, sm: 2, md: 2 }} className="display_center">
                                                                <Grid item xs={4} md={6} className="text-right pt-0">
                                                ...
                                                <Grid item xs={12} className="pt-5">
                                                    <Grid container spacing={2} columnSpacing={{ xs: 1, sm: 2, md: 2 }} className="display_center">
                                                        <Grid item xs={4} md={6} className="text-right pt-0">
                                                            <span className='tax-text'>Tax Amount</span>
                                                        </Grid>
                                                        <Grid item xs={8} md={6} className="pt-0">
                                                            <TextField
                                                                className='ft-amount'
                                                                ...
                                                                <Grid item xs={8} md={6} className="pt-0">
                                                                    <TextField
                                                                        ...
                                                                        InputProps={{
                                                                            startAdornment: <InputAdornment position="start">₹</InputAdornment>,
                                                                            className: "non_disable_text",
                                                                            endAdornment: <Checkbox checked={formValues.pay_from_advance} onChange={this.handleAdvance} />,
                                                                            disabled: true
                                                                        }}
                                                                    />
                                                                </Grid>
                                                    ...
                                                    <Grid container spacing={2} className="display_center">
                                                        ...
                                                        <Grid item xs={8} md={6} className="pt-0">
                                                            <TextField
                                                                ...
                                                                onChange={(e) => this.setState({ return_amount: e.target.value })}
                                                                InputProps={{
                                                                    startAdornment: <InputAdornment position="start">₹</InputAdornment>,
                                                                    className: "non_disable_text"
                                                                }}
                                    ...
                                    }
                                    {
                                        !isReturn ?
                                                    ...
                                                    parseFloat(formValues.due_amount) > 0 ?
                                                        <>
                                                            ...
                                                            <Grid item xs={12} className='p-invoice-date create-input pt-5'>
                                                                <Grid container spacing={2} columnSpacing={{ xs: 1, sm: 2, md: 2 }} className="display_center">
                                                                    <Grid item xs={4} md={6} className="text-right pt-0">
                                                                        <span className='tax-text'> Settlement Date </span>
                                                                    </Grid>
                                                                    <Grid item xs={8} md={6} className="pt-0">
                                                                        <LocalizationProvider dateAdapter={AdapterDayjs}>
                                                                            <DatePicker
                                            ...
                                            :
                                            <Grid item xs={12} md={12} className='p-invoice-date create-input pt-5'>
                                                <Grid container spacing={2} className="display_center">
                                                    <Grid item xs={4} md={6} className="text-right pt-0">
                                                        Return Date
                                                    </Grid>
                    ...
                    <Grid item xs={this.state.isAssign ? 12 : 12} md={this.state.isAssign ? 4 : 12}>
                        <Stack spacing={1} direction="row" className='ratn-footer-buttons' justifyContent="flex-end" style={{ paddingRight: '16px', paddingBottom: '16px' }}>
                            {
                                isEmpty(this.props.query.get('sale_on_approval')) && !this.state.order_id && !this.state.isAssign && this.state.isCreateFrom ?
                                    <LoadingButton
                                        ...
                                        >
                                            {
                                                !will_return_charge_apply ?
                                                    <FormControlLabel value="advance" control={<Radio />} label={"Payment move to advance " + (displayAmount(return_from_wallet))} />
                                                    : null
                                            }
                                            <FormControlLabel value="return" control={<Radio />} label={"Payment Return " + (displayAmount(return_from_wallet))} />
                                        </RadioGroup>
                                    </FormControl>
                                    {
                                        this.state.payment_type == "return" && !will_return_charge_apply ?
                                            <FormControl fullWidth>
