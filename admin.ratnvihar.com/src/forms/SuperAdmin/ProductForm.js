...
import { Box, TextField, Button, Grid, Link, TextareaAutosize, Stack, Select, MenuItem, InputLabel, FormControl, FormControlLabel, Checkbox, FormHelperText, ToggleButtonGroup, ToggleButton, FormLabel, ImageList, ImageListItem, ListItemText, Accordion, AccordionSummary, Typography, AccordionDetails, Paper, Tab } from '@mui/material';
...
class ProductForm extends React.Component {
  ...
  initializeFormData = () => {
    ...
    this.props.actions.materialList({ all: 1, category_id: formValues.category_id });
    this.props.actions.sizeList({ all: 1, sub_category_id: formValues.sub_category_id });
    ...
    this.setState({
      ...
      description: this.state.formData.description ? EditorState.createWithContent(
  ...
  renderCertificatesField = ({
    ...
    <FormControl fullWidth error={touched && error ? true : false}>
      ...
      <Select
        ...
        value={input.value === "" ? [] : input.value}
        multi="true"
        renderValue={(selected) => this.getSelectedCertiNames(selected).join(", ")}
      >
  ...
  renderMaterialsField = ({
    ...
    <FormControl fullWidth error={touched && error ? true : false}>
      ...
      <Select
        ...
        value={input.value === "" ? [] : input.value}
        multi="true"
        renderValue={(selected) => this.getSelectedMaterialNames(selected).join(", ")}
      >
      ...
      >
        <MenuItem value=""></MenuItem>
        {
          this.state.sizeList.map((item, index) => {
            return <MenuItem value={item.id} key={index} className='multi-select'>
              <Checkbox checked={(input.value && input.value.indexOf(item.id) > -1) ? true : false} />
              <ListItemText primary={item.name} />
            </MenuItem>
          })
  ...
  renderFeaturedField = ({
    ...
    <FormControl fullWidth error={touched && error ? true : false} className="featured_field">
      {
        label ?
          <InputLabel>{label}</InputLabel>
          : null
  ...
  onImageChange = (e) => {
    ...
    let totalImages = this.state.existing_images.length + images.length + e.target.files.length;
    if (totalImages > 5) {
      this.props.enqueueSnackbar('Maximum 5 image are allowed.', { variant: 'error' });
      return;
    }
  ...
  handleFormSubmit = async (data, dispatch) => {
    let errors = false;
    let totalImages = this.state.existing_images.length + this.state.images.length;
    if (totalImages == 0) {
      this.props.enqueueSnackbar('Please select at least one image', { variant: 'error' });
      errors = true;
    }

    let values = { ...this.getDefaultValues(), ...data };
    values.description = this.state.description ? draftToHtml(convertToRaw(this.state.description.getCurrentContent())) : "";
    ...
    if (this.state.product_type != 'material') {
      if (values.sizes.length == 0) {
        this.props.dispatch(updateSyncErrors('ProductForm', {
    ...
    for (let i = 0; i < this.state.size_materials.length; i++) {
      for (let x = 0; x < this.state.size_materials[i].materials.length; x++) {
        let plist = this.getMaterialPurities(this.state.size_materials[i].materials[x].material_id);
        let pvalues = this.state.size_materials[i].materials[x].purities;
        this.state.size_materials[i].materials[x].purities = this.getFilteredPurities(plist, pvalues);
      }
    }
  ...
  intializeWeightsTable = (sizes, materials) => {
    ...
    if (this.state.product_type == "material") {
      ...
      if (Array.isArray(sizes) && Array.isArray(materials) && sizes.length && materials.length) {
        for (let i of sizes) {
          let thisMaterials = [];
  ...
  render() {
    ...
    return (
      <form onSubmit={handleSubmit(this.handleFormSubmit)} className="ratn-dialog-wrapper product_form" ref={this.formRef}>
              ...
              this.state.product_type == 'material' ?
                <Grid item xs={6} md={3} className='create-input'>
                  <Field
                    ...
                    label="Material"
                    type="select"
                    onChange={(event, vl) => this.handleSingleMatreialChange(event, vl)}
                    disabled={onlyView}
                  />
                </Grid>
                : null
                ...
                <React.Fragment>
                  <Grid item xs={6} md={3} className='create-input'>
                    <Field
                      ...
                      type="select"
                      defaultValue={[]}
                      onChange={(event, val) => this.handleMaterialsChange(event, val)}
                      disabled={onlyView}
                    />
                  </Grid>
                  <Grid item xs={6} md={3} className='create-input'>
                      ...
                      this.state.size_materials.map((item, index) => (
                        <TableRow key={index}>
                          ...
                          <TableCell>
                            {
                              item.materials.map((m, key) => (
                                <FormControl fullWidth error={('purity_error' in m && m.purity_error) ? true : false} key={key} sx={{ marginBottom: '5px' }} size="small">
                                  <Select
                                    label=""
                                    fullWidth
                                    defaultValue={[]}
                                    multiple
                                    value={m.purities}
                                    renderValue={(selected) => this.getSelectedPurities(selected, m.material_id).join(", ")}
                                    onChange={(e) => this.handlePurityChange(e, index, key)}
                                    disabled={onlyView}
                                  >
                                    <MenuItem value=""></MenuItem>
                                    {
                                      this.getMaterialPurities(m.material_id).map((p, index2) => {
                                        return <MenuItem value={p.id} key={index2} className='multi-select'>
                                          <Checkbox checked={(m.purities && m.purities.indexOf(p.id) > -1) ? true : false} />
                          ...
                          <TableCell>
                            {
                              item.materials.map((m, key) => (
                                <TextField
                                  ...
                                  fullWidth
                                  value={m.weight}
                                  onChange={(e) => this.handleWeightChange(e, index, key)}
                                  sx={{ marginBottom: '5px' }}
                                  error={('weight_error' in m && m.weight_error) ? true : false}
                                  size="small"
                                  disabled={onlyView}
                                />
                              ))
                          ...
                          <TableCell>
                            {
                              item.materials.map((m, key) => (
                                <FormControl fullWidth error={('unit_error' in m && m.unit_error) ? true : false} key={key} sx={{ marginBottom: '5px' }} size="small">
                                  <Select
                                    label=""
                                    fullWidth
                                    defaultValue=""
                                    value={m.unit_id}
                                    onChange={(e) => this.handleUnitChange(e, index, key)}
                                    disabled={onlyView}
                                  >
                                    <MenuItem value=""></MenuItem>
                                    {
                                      this.state.unitList.map((p, index2) => {
                                        return <MenuItem value={p.id} key={index2}>
                                          {p.name}
                                        </MenuItem>
                                      })
                                    }
                              ...
                              <TableCell>
                                {
                                  item.materials.map((m, key) => (
                                    <TextField
                                      ...
                                      fullWidth
                                      value={m.quantity}
                                      onChange={(e) => this.handleQuantityChange(e, index, key)}
                                      sx={{ marginBottom: '5px' }}
                                      error={('qty_error' in m && m.qty_error) ? true : false}
                                      size="small"
                                      disabled={onlyView}
                                    />
                                  ))
                  ...
                  startAdornment: (
                    <Box sx={{ margin: "0 0.2rem 0 0", display: "flex" }}>
                      {this.state.tags.map((data, index) => {
                        return (
                          <Tags data={data} handleDelete={this.handleTagDelete} key={index} />
                        );
                      })}
                    </Box>
                  ),
            ...
            <Grid item xs={12} className='create-input'>
              <Accordion>
                <AccordionSummary
                  ...
                  style={{ padding: 0 }}
                >
                  <Typography style={{ color: "#1e2746", width: "100%", textAlign: "right" }}>SEO Details</Typography>
                </AccordionSummary>
                <AccordionDetails>
            ...
            <div className='custom-row pl-0'>
              <div className='custom-col-2'>
                <div className="admin-buttons">
                  ...
                  this.state.main_image || this.state.existing_main_image ?
                          ...
                          <>
                            {
                              this.state.main_image_file ?
                                <img src={this.getImageSrc(this.state.main_image_file)} />
                                :
                                <img src={noImage} />
                            }
                          </>
                    ...
                    <Button variant="contained" className='image-button' component="label" endIcon={<CloudUploadIcon />} disabled={onlyView}>
                      Main
                      <input
                        name="main_image"
                        hidden
                          ...
                          <>
                            {
                              firstAllImage.type == "existing" ?
                                <button className='close_img' onClick={() => this.deleteExistingImage(0)} type="button" disabled={onlyView}>x</button>
                                :
                                <button className='close_img' onClick={() => this.deleteImage(0)} type="button" disabled={onlyView}>x</button>
                            }
                          </>
                          : null
                      }
                      <img src={firstAllImage.type == "new" ? this.getImageSrc(firstAllImage.path) : firstAllImage.path} />
                    </div>
                    <Button variant="contained" disabled={onlyView} className='image-button' component="label" endIcon={<CloudUploadIcon />}>
                      All Image
                      <input
                        name="images"
                        hidden
                        ...
                        <div>
                          {
                            item.type != "no_image" ?
                              <>
                                {
                                  item.type == "existing" ?
                                    <button className='close_img' disabled={onlyView} onClick={() => this.deleteExistingImage(index + 1)} type="button">x</button>
                                    :
                                    <button className='close_img' disabled={onlyView} onClick={() => this.deleteImage(index + 1)} type="button">x</button>
                                }
                              </>
                              : null
                          }
                          <img src={item.type == "new" ? this.getImageSrc(item.path) : item.path} id="logo-img" />
                        </div>
                      </div>
                    ))
                  }
              ...
              <div className='custom-col-2'>
...

                {
                  ...
                  <div className='p-single-image'>
                    {
                      this.state.existing_video_file ?
                        <>
                          <button className='close_img' onClick={this.deleteExistingVideo} type="button" disabled={onlyView}>x</button>
                          <video
                            width="200"
                            height="150"
                            style={{ height: "143px", objectFit: "contain" }}
                        ...
                        <>
                          {
                            this.state.video_preview ?
                              <>
                                <button className='close_img' onClick={this.deleteVideo} type="button" disabled={onlyView}>x</button>
                                <video
                                  width="100%"
                                  height="140"
                                  style={{ height: "143px", objectFit: "contain" }}
                  ...
                  <Button variant="contained" className='video-button' component="label" disabled={onlyView} endIcon={<CloudUploadIcon />}>
                    Video
                    <input
                      name="video"
                      hidden
          ...
          <Grid container spacing={0} className="loans_view p_view">
            <Grid item xs={12} className='create-input' style={{ paddingTop: "10px" }}>
              <Stack spacing={1} direction="row" justifyContent="flex-end" className='p-submit-button' sx={{ marginTop: "0px" }}>
                ...
                {
                  !submitting ?
                    <Button variant="outlined" onClick={() => this.props.navigate(getUserDashboardRoute(getRoleName(this.state.auth)) + '/products')}>Cancel</Button>
                    : null
                }
              </Stack>
            </Grid>
