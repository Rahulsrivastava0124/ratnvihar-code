...
class SubCategoryForm extends React.Component {
  ...
  renderIncreaseField = ({
    ...
    <TextField
      InputProps={{
        endAdornment: <InputAdornment position="end">% &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{custom.making_charge}</InputAdornment>
      }}
      label={label}
      fullWidth
      error={touched && error ? true : false}
  ...
  renderDiscountField = ({
    ...
    <FormControl fullWidth sx={{ m: 1 }}>
      <TextField
          variant="outlined"
          InputProps={{
              endAdornment: <InputAdornment position="end">% &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{custom.discounted_price}</InputAdornment>
          }}
          label={label}
          fullWidth
          error={touched && error ? true : false}
  ...
  calculatePrice = () => {
    let {base_price, increase, admin_discount, distributor_discount, retailer_discount, customer_discount} = this.props.formValues;
    let making_charge = '', admin_discounted_price = '', distributor_discounted_price = '', retailer_discounted_price = '', customer_discounted_price = '';
    base_price = base_price ? parseFloat(base_price) : 0;
    increase = increase ? parseFloat(increase) : 0;
    if(base_price > 0){
      let increase_amt = increase > 0 ? priceFormat(base_price * 100 / increase) : base_price;
      making_charge = increase_amt; //priceFormat(base_price + increase_amt);
      admin_discounted_price = admin_discount ? priceFormat(making_charge - (making_charge * parseFloat(admin_discount) / 100)) : making_charge;
      distributor_discounted_price = distributor_discount ? priceFormat(making_charge - (making_charge * parseFloat(distributor_discount) / 100)) : making_charge;
      retailer_discounted_price = retailer_discount ? priceFormat(making_charge - (making_charge * parseFloat(retailer_discount) / 100)) : making_charge;
      customer_discounted_price = customer_discount ? priceFormat(making_charge - (making_charge * parseFloat(customer_discount) / 100)) : making_charge;
    }else{
      making_charge = 0;
      admin_discounted_price = '';
      distributor_discounted_price = '';
  ...
  loadSubCategoryList = async(category_id) => {
    if(category_id){
      let res = await subCategoryRawList({category_id: category_id ? category_id : ''});
      if(res.data.success){
        this.setState({
  ...
  render() {
    ...
    let making_charge_type = formValues && 'making_charge_type' in formValues ? formValues.making_charge_type : '';
    let making_charge = formValues && 'making_charge' in formValues ? formValues.making_charge : '';
    return (
    ...
    /> 
              ...
              <Grid item xs={12} md={3} className='create-input'>
                <Field
                  ...
                  label="Admin Disc"
                  onChange={this.handlePriceChange}
                  discounted_price={displayAmount(this.state.admin_discounted_price)}
                />
              </Grid>
              <Grid item xs={12} md={3} className='create-input'>
                <Field
                  ...
                  onChange={this.handlePriceChange}
                  discounted_price={displayAmount(this.state.distributor_discounted_price)}
                />
              </Grid>
              <Grid item xs={12} md={3} className='create-input'>
                <Field
                  ...
                  label="Retailer Disc"
                  onChange={this.handlePriceChange}
                  discounted_price={displayAmount(this.state.retailer_discounted_price)}
                />
              </Grid>
              <Grid item xs={12} md={3} className='create-input'>
                <Field
                  ...
                  label="Customer Disc"
                  onChange={this.handlePriceChange}
                  discounted_price={displayAmount(this.state.customer_discounted_price)}
                />
              </Grid>
              <Grid item xs={12}>
