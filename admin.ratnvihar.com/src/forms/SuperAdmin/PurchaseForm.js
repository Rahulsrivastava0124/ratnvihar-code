...
import { Box, TextField, Button, Grid, Link, TextareaAutosize, Stack, Select, MenuItem, InputLabel, FormControl, FormControlLabel, Checkbox, FormHelperText, ToggleButtonGroup, Typography, Collapse, ImageList, ImageListItem, InputAdornment, IconButton, Autocomplete, RadioGroup, Radio, Alert, Accordion, AccordionSummary } from '@mui/material';
...
import { purchaseStore, purchaseUpdate, purchaseNewInvoiceNumber, purchaseReturn, purchaseRawEdit } from 'actions/superadmin/purchase.actions';
...
class PurchaseForm extends React.Component {
    ...
    loadProductsFromOnApprove = async () => {
        if (!isEmpty(this.props.query.get('purchase_on_approve')) && this.props.query.get('purchase_on_approve') != 0) {
            let res = await purchaseRawEdit(this.props.query.get('purchase_on_approve'));
            if (res.data.success) {
                this.setState({
                    formValues: {
                    ...
                    setTimeout(() => {
                        this.handleSupplierChange({target: {value: res.data.data.supplier_id}})
                    }, 1000);
                })
            }
        }
    ...
    loadReturnSale = async() => {
        if (!isEmpty(this.props.query.get('return_sale')) && this.props.query.get('return_sale') != 0) {
            ...
            if(res.data.success){
                this.setState({
                    ...
                    formValues: {
                        ...this.state.formValues,
                        paid_amount: res.data.data.return_amount_from_wallet > 0 ? res.data.data.return_amount_from_wallet : this.state.formValues.paid_amount
                    }
                })
            }
        }
    ...
    componentDidUpdate(prevProps) {
        ...
        if (this.props.query.get('purchase_on_approve') != prevProps.query.get('purchase_on_approve')) {
            this.loadProductsFromOnApprove();
        }

        if (this.props.query.get('return_sale') != prevProps.query.get('return_sale')) {
            this.loadReturnSale();
        }
            ...
            if (this.state.isCreateFrom) {
                if (this.state.createSuccess) {
                    ...
                    if (this.props.query.get('purchase_on_approval') == 0) {
                        this.props.navigate(getUserDashboardRoute(getRoleName(this.state.auth)) + '/purchase-on-approve');
                    } else {
    ...
    handleSupplierChange = (event) => {
        ...
        let supplier_gst_no = '', advance_amount = 0;
        if (m.length) {
    ...
    updateProductFormValues = (val, key) => {
        ...
        if (key == "product_id") {
            ...
            productFormValues.has_certificate = m.length ? (m[0].certificates.length ? true : false) : false;
        ...
        if (!isEmpty(formData.product_id)) {
            ...
            if (m.length) {
                ...
                if (product.type == "material") {
                    materials[0] = { ...materials[0], weight: product.size_materials[0].materials[0].weight };
                    materials[0] = { ...materials[0], quantity: product.size_materials[0].materials[0].quantity };
                    materials[0] = { ...materials[0], unit_id: product.size_materials[0].materials[0].unit_id };
                    materials[0] = { ...materials[0], unit_name: product.size_materials[0].materials[0].unit_name };
                    ...
                    if (!isEmpty(formData.size_id)) {
                        let sizeArr = _.filter(product.size_materials, { size_id: formData.size_id });
                        if (sizeArr.length) {
                            let size = sizeArr[0];
                            for (let i = 0; i < materials.length; i++) {
                                let sm = _.filter(size.materials, { material_id: materials[i].material_id });
                                if (sm.length) {
                                    ...
                                    materials[i] = { ...materials[i], unit_name: sm[0].unit_name };
                                }
                            }
                        }
                    }
    ...
    productFormValidate = () => {
        ...
        if (productFormValues.product_type != 'material' && productFormValues.has_certificate) {
    ...
    handleSubmit = () => {
        ...
        if (!hasErr && formValues.products.length) {
            ...
            if (this.state.isCreateFrom) {
                let data = { ...this.state.formValues, on_approval: this.props.query.get('purchase_on_approval') == 0 ? true : false, on_approval_id: this.props.query.get('purchase_on_approve'), return_sale_id: return_sale_data ? return_sale_data.id : '' }
    ...
    handleSubCategoryChange = (event) => {
        ...
        if (this.state.formValues.type == "material") {
            if (event.target.value) {
                this.loadSizeMaterialsForMaterial({ sub_category_id: event.target.value });
            } else {
                this.setState({
                    sizeList: []
                })
            ...
            this.props.actions.productList({ all: 1, sub_category_id: event.target.value, purity_price: 1 });
            this.updateProductFormValues("", 'product_id');
            this.props.dispatch({
                type: RESET_PRODUCT_LIST
            })
    ...
    isMaterialFormDisabled = () => {
        if (this.state.formValues.products.length == 0 && !this.state.productFormValues.size_id && this.state.productFormValues.product_type != 'material') {
            return true;
        } else {
            return false;
        }
    ...
    render() {
        const { formValues, formErros, productFormValues, productFormErros, materialFormErros, submitting, actionProductIndex, return_sale_data } = this.state;
            ...
            <Box sx={{ flexGrow: 1, m: 0.5 }} className='ratn-dialog-inner'>
                            ...
                            <Grid item xs={12} md={12} className='create-input'>
                                <Accordion className='rtn_accordion'>
                                    <AccordionSummary
                                        ...
                                        id="panel1a-header"
                                    >
                                        <Typography>Return Sale # {return_sale_data.invoice_number} | {`${return_sale_data.user_name}, ${return_sale_data.user_mobile}`} | {return_sale_data.return_date}</Typography>
                                    </AccordionSummary>
                                </Accordion>
                            </Grid>
                            : null
                ...
                <Grid container spacing={2} className='tax-input loans_view p_view'>
                    {
                        !this.state.isCreateFrom ?
                            <Grid item xs={!formValues.supplier_id ? 12 : 12} md={!formValues.supplier_id ? 6 : 4} className='create-input'>
                                <TextField
                                    label="Supplier"
                                    variant="outlined"
                                    fullWidth
                            ...
                            <Grid item xs={!formValues.supplier_id ? 12 : 12} md={!formValues.supplier_id ? 6 : 4} className='create-input'>
                                <FormControl fullWidth error={formErros.supplier_id}>
                                    ...
                                    <Select
                                        ...
                                        disabled={!this.state.isCreateFrom || (!isEmpty(this.props.query.get('purchase_on_approve')) ? true : false)}
                                    >
                    ...
                    <Grid item xs={!formValues.supplier_id ? 12 : 12} md={!formValues.supplier_id ? 3 : 2} className='p-invoice-date create-input'>
                ...
                <Grid container spacing={2} className='loans_view tax-input'>
                    <Grid item xs={12} md={12} className='p-add-product create-input'>
                        <h3 className='p_heading_list mb-0 mt-0' style={{ position: 'relative' }}>
                            {
                                !this.state.isReturnForm ?
                                    <>
                                        ...
                                        <Button variant="contained" className='add-button purchase_add_p' onClick={() => this.handleAddNewProduct()} style={{ width: "140px" }}>Add Product</Button>
                                    </>
                                    : null
                            }
                            <span className='purchase_p_title'>Purchase {formValues.type == "product" ? "Products" : "Materials"}</span>
                        </h3>
                        {
                            this.state.productDialog ?
                                <Box sx={{ flexGrow: 1, m: 0.5 }}>
                                    <Grid container spacing={2} className='loans_view tax-input p_view'>
                                        ...
                                        <Grid item xs={productFormValues.product_type == "in_house" ? 12 : 12} md={productFormValues.product_type == "in_house" ? 3 : 3}>
                                        ...
                                        </FormControl>*/}
                                            {
                                                formValues.type == "product" ?
                                                    <Autocomplete
                                                        ...
                                                        renderOption={(props, option) => <li {...props} key={option.id}>{option.name + " | " + option.product_code}</li>}
                                                        renderInput={(params) => (
                                                            <TextField
                                                                {...params}
                                                                label="Product"
                                                    ...
                                                    <Autocomplete
                                                        ...
                                                        onChange={(event, newValue) => {
                                                            this.updateProductFormValues(newValue ? newValue.id : '', 'material_id');
                                                        }}
                                                    />
                                            }
                                        </Grid>
                                        {
                                            (productFormValues.product_type != "material"/* || formValues.type == "material"*/) ?
                                                <>
                                                    {
                                                        productFormValues.product_type != "material" && formValues.type != "material" && productFormValues.has_certificate ?
                                                            <Grid item xs={12} md={2} >
                                                                <TextField
                                                                            ...
                                                                            >
                                                                                <MenuItem value=""></MenuItem>
                                                                                {
                                                                                    item.purities.map((item, index) => (
                                                                                        <MenuItem value={item.id} key={index}>{item.name} {item.mrp_display ? <>({displayAmount(item.mrp_display)} / {item.unit_name})</> : ''}</MenuItem>
                                                                                    ))
                                                                                }
                                                                            </Select>
                                                                        </FormControl>
                                ...
                                <TableBody>
                                    {
                                        formValues.products.map((item, index) => (
                                            <React.Fragment key={index} >
                                                    ...
                                                    <TableCell>
                                                        {
                                                            item.materials.map((m, key) => (
                                                                <p key={key} className="purchase-material m-0" style={{ color: "#000" }}>
                                                                    {m.material_name}{this.getMaterialPurity(m.purities, m.purity_id, m.purity_name)} &nbsp; {(this.state.isReturnForm && m.quantity) ? <>{m.avl_qty - parseInt(m.return_qty ? m.return_qty : 0)} Qty&nbsp;</> : null} {m.weight} &nbsp; {m.unit_name} &nbsp; x &nbsp; {m.rate}
                                                                </p>
                                                            ))
                                                        }
                                                    </TableCell>
                                                ...
                                                {
                                                    (this.state.isReturnForm && item.materials.length == 1 && this.checkOpen(item.id)) ?
                                    ...
                                    }
                                    {
                                        formValues.products.length > 0 ?
                                            <TableRow>
                                                <TableCell colSpan={!this.state.isReturnForm ? 8 : 9}></TableCell>
                                                <TableCell>
                                                    <b>Sub Total<br />
                    ...
                    <>
                        ...
                        <Grid item xs={12} md={4} className='create-input pt-0' style={{ paddingRight: '16px' }}>
                            <Grid container spacing={2} className='mob_responsive_purchase_input'>
                                {/*<Grid item xs={12}>
                                <TextField
                                    label="Sub Total"
                                        ...
                                        <Grid item xs={12} md={6} className="pt-0">
                                            <FormControl fullWidth className='ft-amount'>
                                                ...
                                                >
                                                    ...
                                                    <MenuItem value="cheque" disabled={return_sale_data ? true : false}>Cheque</MenuItem>
                                                ...
                                                <Grid container spacing={2} className="display_center">
                                                    ...
                                                    <Grid item xs={12} md={6} className="pt-0">
                                                        <TextField
                                                            ...
                                                            disabled={!this.state.isCreateFrom || (return_sale_data ? true : false)}
                                                            InputProps={{
                                                                startAdornment: <InputAdornment position="start">₹</InputAdornment>,
                                                                className: "non_disable_text"
                                                            }}
                            ...
                            <Stack spacing={1} direction="row" className='ratn-footer-buttons' justifyContent="flex-end" style={{ paddingRight: '16px', paddingBottom: '16px' }}>
                                {
                                    !this.state.isReturnForm ?
                                        ...
                                        >
                                            {this.props.query.get('purchase_on_approval') == 0 ? 'Purchase On Approval' : 'Submit'}
                                        </LoadingButton>
                                        :
                                        <>
                                            {
                ...
                >
                    ...
                    <DialogContent>
                        ...
                        {
                            ((this.state.formValues.due_amount && parseFloat(this.state.return_amount) > parseFloat(this.state.formValues.due_amount))) ?
                                <>
                                    <FormControl>
                                        <RadioGroup
                                            row
                                        ...
                                        >
                                            <FormControlLabel value="advance" control={<Radio />} label={"Payment move to advance " + (displayAmount(parseFloat(this.state.return_amount) - parseFloat(this.state.formValues.due_amount)))} />
                                            <FormControlLabel value="return" control={<Radio />} label={"Payment Return " + (displayAmount(parseFloat(this.state.return_amount) - parseFloat(this.state.formValues.due_amount)))} />
                                        </RadioGroup>
                                    </FormControl>
                                    {
