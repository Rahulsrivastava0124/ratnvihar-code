      ...
      >
        <option value=""></option>
        {
          this.getSubCategoryByGroup().map((item, index) => {
            return <optgroup label={item.category_name} key={index}>
              {
                item.subcategories.map((s, key) => {
                  return <option value={s.id} key={key}>{s.name}</option>
                })
              }
            </optgroup>
          })
    ...
    return (
      <form onSubmit={ handleSubmit }>
         <Box sx={{ flexGrow: 1 }}>
            <Grid container spacing={2}>
              <Grid item xs={6} className='create-input'>
                <Field
                  ...
                  label="Sub Category"
                  type="select"
                  onChange={(event, val) => this.props.subCategoryChange(val)}
                />
              </Grid>
              <Grid item xs={6} className='create-input'>
                <Field
            ...
            <Stack spacing={1} mt={2} direction="row" className='modal-button-area ratn-footer-buttons'>
              {
                this.state.formData ?
